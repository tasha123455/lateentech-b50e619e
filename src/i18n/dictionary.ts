// English -> Arabic dictionary. Only strings present here are translated.
// Underlying data (currencies, countries, user-typed values, numbers, prices,
// toggle states, form inputs) stays intact — we only swap visible UI text.
export const DICT: Record<string, string> = {
  // ===== Landing =====
  "Performance marketing, zero upfront cost": "تسويق بالأداء، من غير أي تكلفة مسبقة",
  "Wasla — Your smartest link to modern commerce": "وصلة — صِلَتك الأقوى للتجارة الذكية",
  "Your smartest link to modern commerce": "صِلَتك الأقوى للتجارة الذكية",
  "Who are you?": "من إنت؟",
  "Where can I find it?": "وين نلقى حاجتي؟",
  "Browse products & buy easily": "تصفح المنتجات واشتري بسهولة",
  "Promote products & earn your commissions instantly": "سوّق للمنتجات و أكسب و أنت في مكانك",
  "List your products and let marketers increase your sales": "اعرض منتجاتك وخلي المسوّق يزيد مبيعاتك",
  "New here? Selecting a role will walk you through sign-up.": "جديد معانا؟ اختار نوع حسابك وحنمشوا معاك خطوة بخطوة للتسجيل.",
  "By continuing you agree to our": "باستمرارك، إنت توافق على",
  "Terms": "الشروط والأحكام",
  "Privacy Policy": "سياسة الخصوصية",

  // ===== Global & Navigation =====
  "Menu": "القائمة",
  "Your points and prizes": "نقاطك و الجوائز",
  "Home": "الرئيسية",
  "Browse": "تصفح المنتجات",
  "Browse products": "تصفح المنتجات",
  "Products": "المنتجات",
  "My products": "منتجاتي",
  "Orders": "الطلبات",
  "Customer name": "إسم الزبون",
  "Country code": "كود الدولة",
  "Optional WhatsApp": "رقم الواتساب (اختياري)",
  "Write any extra instructions, like preferred delivery time…": "أكتب أي تعليمات إضافية، زي وقت التوصيل،إلخ....",
  "Saved": "المحفوظة",
  "Notifications": "الإشعارات",
  "Sign out": "تسجيل الخروج",
  "Back": "رجوع",
  "‹ Back": "‹ رجوع",
  "Close": "إغلاق",
  "Cancel": "إلغاء",
  "Apply": "تأكيد",
  "Reset all": "إعادة الإعدادات الإفتراضية",
  "Browse with pictures": "تصفح بالصور",
 "soon": "قريباً",
 "Soon": "قريباً",
 "More": "المزيد",
 "More countries": "دول أخرى",
 "OK": "حسناً",
 "— More — Soon": "— المزيد — قريباً",
  "Change": "تغيير",
  "Remove": "حذف",
  "Delete": "مسح",
  "Edit": "تعديل",
  "Save": "حفظ",
  "Save as Draft": "حفظ كمسودة",
  "Save as draft": "حفظ كمسودة",
  "Copy": "نسخ",
  "Copy link": "نسخ الرابط",
  "Get my link": "احصل على الرابط الخاص بي",
  "Copied!": "تم النسخ!",
  "Hey": "مرحباً",
  "Hey there": "أهلاً بك",

  // ===== Auth =====
  "Welcome back": "مرحبتين بيك من جديد",
  "Sign in to your marketer account": "سجّل دخولك إلى حسابك كمسوّق",
  "Sign in to your business account": "سجّل دخولك إلى حسابك كصاحب عمل",
  "Create account": "إنشاء حساب",
  "Creating…": "جارٍ الإنشاء…",
  "Free to join — earn on every sale you drive": "الانضمام مجاني — اكسب على كل عملية بيع",
  "List your products and let marketers grow your sales": "اعرض منتجاتك وخلّي المسوّقين يزيدوا مبيعاتك",
  "Email": "البريد الإلكتروني",
  "Password": "كلمة المرور",
  "Show": "إظهار",
  "Hide": "إخفاء",
  "Forgot password?": "نسيت كلمة المرور؟",
  "Sign in": "تسجيل الدخول",
  "Signing in…": "جارٍ تسجيل الدخول…",
  "or": "أو",
  "Continue with Google": "الاستمرار باستخدام جوجل",
  "No account?": "ما عندكش حساب؟",
  "Create one": "دير حساب جديد",
  "New here? Create an account": "معندکش حساب ؟ إنشاء حساب جدید",
  "New here?": "معندکش حساب ؟",
  "Create an account": "إنشاء حساب جدید",
  "Already have an account?": "عندك حساب؟",
  "Have an account?": "عندك حساب؟",
  "Sign in with Google": "تسجيل الدخول بواسطة جوجل",
  "No existing account for this Google email. Please sign up first.": "لا يوجد حساب موجود بهذا البريد الإلكتروني من جوجل. يرجى إنشاء حساب جديد أولاً.",
  "An account with this email already exists. Please sign in instead.": "يوجد حساب بهذا البريد الإلكتروني. جرب تسجيل الدخول",
  "Phone must be 10 digits and start with 091, 092, 093, or 094.": "يجب أن يتكون رقم الهاتف من 10 أرقام ويبدأ بـ 091 أو 092 أو 093 أو 094",
  "More countries soon": "دول أخرى قريباً",
  "Full name": "الاسم الكامل",
  "Name": "الاسم الكامل",
  "Phone": "رقم الهاتف",
  "Phone number": "رقم الهاتف",
  "Business name": "اسم تاجر / النشاط التجاري",
  "Go to dashboard →": "الانتقال إلى لوحة التحكم ←",
  "Marketer": "مسوّق",
  "Business": "تاجر",
  "business owner": "صاحب العمل",
  "Business owner": "صاحب العمل",

  // ===== Business Profile & Settings =====
  "Business Profile": "ملف تاجر",
  "Business Info": "معلومات تاجر",
  "Company Name": "اسم تاجر",
  "Industry": "مجال العمل",
  "Fashion & Apparel": "أزياء وملابس",
  "Company website (optional)": "موقع تاجر (اختياري)",
  "Contact Info": "معلومات التواصل",
  "Support email": "إيميل الدعم الفني",
  "Support phone": "رقم هاتف الدعم",
  "Location": "الموقع",
  "Headquarters Country": "دولة المقر",
  "Headquarters City": "مدينة المقر",
  "Save Profile": "حفظ الملف",

  // ===== Shipping Countries & Zones =====
  "Shipping Countries": "دول الشحن",
  "Set shipping and delivery cost per city.": "حدد تكلفة الشحن والتوصيل لكل مدينة.",
  "+ Add country": "+ إضافة دولة",
  "Add a country": "+ إضافة دولة",
  "Add city": "إضافة مدينة",
  "Libya": "ليبيا",
  "Tripoli": "طرابلس",
  "Benghazi": "بنغازي",
  "Suluq": "سلوق",
  "Ghemines": "قمينس",
  "Ajdabiya": "اجدابيا",
  "Al Kuwayfiyah": "الكويفيه",
  "Ras Al Minqar": "راس المنقار",
  "Sidi Khalifa": "سي خليفه",
  "Al Abyar": "لبيار",
  "Driana": "دريانه",
  "Bersis": "برسس",
  "Al Mabni": "المبني",
  "Tocra": "توكره",
  "Istatah": "اسطاطه",
  "Al Aweilia": "لعويله",
  "Murad Masud": "مراد مسعود",
  "Al Aquriyah": "العقوريه",
  "Zueitina": "زويتنه",
  "Al Bakkur": "البكور",
  "Al Wardiyah": "الورديه",
  "Farzughah": "فرزوغه",
  "Tacnis": "تاكنس",
  "Battah": "بطه",
  "Al Marj": "المرج",
  "Jardas": "جردس",
  "Zawiyat al Arqub": "زاوية العرقوب",
  "Belhadid": "بلحديد",
  "Qasr Libya": "قصر ليبيا",
  "Wadi al Kuf": "وادي الكوف",
  "Al Bayadah": "البياضه",
  "Marawah": "مراوه",
  "Massah": "مسه",
  "Al Bayda": "البيضاء",
  "Shahat": "شحات",
  "Wardamah": "وردامه",
  "Ras Turab": "راس تراب",
  "Qarnadah": "قرناده",
  "Omar Al Mukhtar": "عمر المختار",
  "Qandulah": "قندوله",
  "Al Haniyah": "الحنيه",
  "Al Wasitah": "الوسيطه",
  "Al Faidiyah": "الفايديه",
  "Al Abraq": "الابرق",
  "Susa": "سوسه",
  "Al Mansurah": "المنصوره",
  "Al Qubbah": "القبه",
  "Belkhather": "بالخاثر",
  "Qardabah": "قرضبه",
  "Tobruk": "طبرق",
  "Bab al Zaytun": "باب الزيتون",
  "Al Qa'rah": "القعره",
  "Kamboot": "كمبوت",
  "Al Watar": "الوتر",
  "Bir al Ashhab": "بئر الاشهب",
  "Bardia": "البردي",
  "Emsaed": "امساعد",
  "Al Jaghbub": "الجغبوب",
  "Ain Marah": "عين ماره",
  "Derna": "درنه",
  "Umm al Rizam": "ام رزم",
  "Martuba": "مرتوبه",
  "Gulf of Bomba": "خليج البمبه",
  "Ras al Helal": "راس الهلال",
  "Kersa": "كرسه",
  "Al Mikhili": "المخيلي",
  "Al Aziyat": "العزيات",
  "Al Tamimi": "التميمي",
  "Ain al Ghazalah": "عين الغزاله",
  "Brega": "البريقه",
  "Al Uqaylah": "العقيله",
  "Ras Lanuf": "راس لانوف",
  "Bin Jawad": "بن جواد",
  "Harawa": "اهراوه",
  "Sirte": "سرت",
  "Tawergha": "تورغاء",
  "Misrata": "مصراته",
  "Zliten": "زليتن",
  "Wadi Kaam": "وادي كعام",
  "Al Khums": "الخمس",
  "Qasr Al Khiyar": "قصر الخيار",
  "Al Alous": "العلوص",
  "Garabulli": "القرابولي",
  "Msallata": "مسلاته",
  "Tarhuna": "ترهونه",
  "Bishr": "بشر",
  "The South": "الجنوب",
  "Sabha": "سبها",
  "Al Kufra": "الكفره",
  "Al Aziziyah": "العزيزية",
  "Gharyan": "غريان",
  "Al Asaba": "الاصابعه",
  "Kikla": "ككله",
  "Yafran": "يفرن",
  "Ar Rayaynah": "الريانيه",
  "Zintan": "الزنتان",
  "Ar Rajban": "الرجبان",
  "Jadu": "جادو",
  "Ar Ruhaybat": "الرحيبات",
  "Al Josh": "الجوش",
  "Jalu": "جالو",
  "Awjila": "اوجله",
  "Bani Walid": "بني وليد",
  "Brak Al Shati": "براك الشاطي",
  "Samnu": "سمنه",
  "Ghudwah": "غدوة",
  "Ubari": "اوباري",
  "Tmassah": "تمسه",
  "Al Awaynat": "العوينات",
  "Ghat": "غات",
  "Murzuq": "مرزق",
  "Traghen": "تراغن",
  "Zawiya": "الزاوية",
  "Al Zahra": "الزهراء",
  "Surman": "صرمان",
  "Sabratha": "صبراته",
  "Al Ajelat": "العجيلات",
  "Al Jumayl": "الجميل",
  "Zuwara": "زواره",
  "Riqdalin": "راقده لين",
  "Zaltan": "زلطن",
  "Badr": "بدر",
  "Tiji": "تيجي",
  "Tamzin": "طمزين",
  "Kabo": "كابو",
  "Nalut": "نالوت",
  "Wazzan": "وزان",
  "Al Qal'ah": "القلعه",
  "Al Sa'diyah": "الساعديه",
  "Delivery cost": "تكلفة التوصيل",
  "Delivery time": "مدة التوصيل",
  "e.g., 2-3 days": "مثال: 2-3 أيام",
  "COD available": "الدفع عند الاستلام (COD) متاح",
  "Shipping delivery": "الشحن والتوصيل",
  "Shipping & delivery": "الشحن والتوصيل",
  "Delivery by city": "التوصيل حسب المدينة",
  "Delivery": "التوصيل",
  "delivery": "التوصيل",
  "Deliv": "التوصيل",
  "Low stock": "المخزون منخفض",
  "Out of stock": "نفذت الكميه",

  // ===== Dashboards: Stats & Wallet =====
  "Business Dashboard": "لوحة تحكم تاجر",
  "Marketer Dashboard": "لوحة تحكم المسوّق",
  "WALLET BALANCE": "رصيد المحفظه",
  "Wallet Balance": "رصيد المحفظه",
  "Available after admin receipt approval": "",
  "Wallet": "المحفظة",
  "wallet": "المحفظة",
  "NET EARNINGS": "صافي الأرباح منذ إنشاء الحساب",
  "Net earnings": "صافي الأرباح",
  "Next payout": "تقدر تسحب بعد",
  "Next payout ": "تقدر تسحب بعد ",
  "Withdraw": "سحب",
  "Withdraw earnings": "طلب سحب الأرباح",
  "Confirm withdrawal": "تأكيد عملية السحب",
  "Withdrawal processed": "جاري معالجة السحب",
  "You can withdraw today": "تقدر تسحب اليوم",
  "Pending withdrawal": "طلب السحب قيد المراجعه",
  "Withdrawal successful": "تم السحب بنجاح",
  "Your withdrawal has been paid.": "تم تحويل المبلغ بنجاح.",
  "Withdrawal request needs attention": "طلب السحب يحتاج إلى مراجعة",
  "Minimum withdraw amount 20 LYD": "اقل قيمه يمكن سحبها 20 د.ل",
  "Requested": "تم تقديم الطلب",
  "pending clearance": "في الانتظار / قيد المراجعة",
  "Pending": "قيد الانتظار",
  "Pending verification by admin": "في انتظار مراجعة الإدارة",
  "Total earned": "إجمالي الأرباح منذ إنشاء الحساب",
  "Gross revenue": "إجمالي الإيرادات",
  "Revenue": "الإيرادات",
  "Earnings": "الأرباح",
  "After commissions & platform fees": "بعد العمولات ورسوم المنصة",
  "This month": "هذا الشهر",
  "View breakdown": "عرض التفاصيل",
  "Financial breakdown": "التفاصيل المالية",
  "Financials": "الماليات",
  "Platform fee": "رسوم المنصة",
  "Platform fees": "رسوم المنصة",
  "Pieces sold": "قطع",
  "Pieces": "القطع",
  "Succeeded": "الطلبات تم تسليمها",
  "Failed": "فشل",
  "Delivered": "تم التسليم",
  "✓ Delivered": "✓ تم التسليم",
  "Full payment received": "تم استلام كامل المبلغ",
  "Awaiting marketer confirmation": "بانتظار تأكيد المسوّق",
  "Manual (marketer)": "يدوي (المسوّق)",
  "(no customer yet)": "(لا يوجد زبون بعد)",
  "Failed (COD)": "طلبات لم يتم استلامها (فشل الدفع عند الاستلام)",
  "Cash-on-delivery failures only": "",
  "failure rate": "نسبة الطلبات غير المستلمة",
  "success": "نسبة النجاح",
  "active": "نشط",
  "Marketers": "المسوّقين",
  "Marketers active:": "المسوقون النشطون:",
  "Available balance": "رصيد المحفظه",
  "Updated just now": "تم التحديث الآن",
  "Payout": "السحب",
  "Payouts": "السحوبات",
  "Money": "المالية",
  "Requests": "الطلبات",
  "What they wrote": "اللي كتبه",
  "Nothing written": "ما كتب شي",
  "Mark as done": "تم التنفيذ",
  "What you did, in a line — they see this as the reason their details changed": "اكتب باختصار اللي عملته — حيشوفه كسبب تغيير بياناته",
  "Receipts": "الإيصالات",
  "Notify": "إشعار",
  "Confirm your payout details below": "أكّد تفاصيل السحب أسفله",
  "Lifetime, from confirmed & delivered orders": "إجمالي الأرباح من الطلبيات المؤكدة والمسلّمة",
  "Commissions": "العمولات",

  // ===== Products & Browse =====
  "Filters": "فلتره",
  "Filter & sort": "فلتره",
  "All": "الكل",
  "Fashion": "ملابس وأحذية",
  "Fashion & footwear": "ملابس وأحذية",
  "Beauty": "عناية وتجميل",
  "Beauty & skincare": "عناية وتجميل",
  "Fitness": "صحة ورياضة",
  "Health & fitness": "صحة ورياضة",
  "Tech": "أجهزة وتقنيات",
  "Tech & gadgets": "أجهزة وتقنيات",
  "Nutrition": "أغذية وتغذية",
  "Food & nutrition": "أغذية وتغذية",
  "Home & living": "مستلزمات المنزل",
  "Recommended for you": "مقترح ليك",
  "All products": "كل المنتجات",
  "Live Products": "المنتجات النشطة",
  "Highest commission first": "أعلى عمولة أولاً",
  "Lowest commission first": "أقل عمولة أولاً",
  "Highest price first": "أعلى سعر أولاً",
  "Lowest price first": "أقل سعر أولاً",
  "Category": "التصنيف",
  "Select": "اختار",
  "In stock": "متوفر في المخزون",
  "Low stock alert": "تنبيه: المخزون قرب يكمل",
  "Stock qty": "الكمية بالمخزون",
  "Sold": "مبيوع",
  "Product code": "كود المنتج",
  "Order Code": "كود الطلبية",
  "Order code": "كود الطلبية",
  "Source": "المصدر",
  "Commission": "نسبة العمولة",
  "Commission %": "نسبة العمولة",
  "Commission rate": "نسبة العمولة",
  "Price per unit": "سعر القطعة",
  "Price": "سعر القطعة",
  "Product price": "سعر القطعة",
  "Selling price": "سعر المنتج",
  "Marketer commission": "عمولة المسوّق",
  "Marketer comm.": "عمولة المسوّق",
  "Marketer commissions": "عمولة المسوّق",
  "Marketer earns": "مكسب المسوّق",
  "Your commission": "مكسبك من كل بيعة",
  "Your earning per sale": "مكسبك من كل بيعة",
  "Your earning per unit": "مكسبك من كل بيعة",
  "By percentage (%)": "بنسبة مئوية (%)",
  "Fixed amount": "مبلغ ثابت",
  "Available colours": "الألوان المتوفرة",
  "Colours": "الألوان المتوفرة",
  "Colour": "الألوان المتوفرة",
  "Available sizes": "المقاسات المتوفرة",
  "Sizes": "المقاسات المتوفرة",
  "Size": "المقاسات المتوفرة",
  "Select colour": "اختار اللون",
  "Select size": "اختار المقاس",
  "Quantity": "الكميه",
  "QTY": "الكمية",
  "Qty": "الكمية",
  "Product name": "اسم المنتج",
  "Description": "الوصف",
  "Product details": "تفاصيل المنتج",
  "Product photos": "صور المنتج",
  "Product": "المنتج",
  "+ Add colour": "+ إضافة لون",
  "+ Add size": "+ إضافة مقاس",
  "Skip if not applicable": "تخطى لو مش ضروري",
  "+ Add": "+ إضافة",
  "Add": "+ إضافة",
  "Add a product": "إضافة منتج",
  "Add product": "إضافة منتج",
  "List product": "عرض المنتج في المنصة",
  "Edit product": "تعديل المنتج",
  "Add photo": "إضافة صورة",
  "Up to 6 photos. First photo is the main image.": "حتى 6 صور. الصورة الأولى هي الأساسية.",
  "Variants": "خيارات المنتج",
  "Currency": "العملة",
  "Select a currency": "اختار العملة",
  "Select a product": "اختار منتج",
  "Auto-generated": "توليد تلقائي",
  "Regenerate": "إعادة توليد",
  "Pause": "إيقاف مؤقت",
  "Paused": "متوقف",
  "Activate": "تفعيل",
  "Manage your catalog": "إدارة كتالوج منتجاتك",
  "Set by the business owner": "محددة بواسطة صاحب العمل",
  "Choose from your saved products": "اختار من منتجاتك المحفوظة",
  "Across every shop": "في كل المتاجر",
  "Available across ": "متوفر في ",
  "Available across two currencies": "متوفر بعملتين",

  // ===== Orders =====
  "Customer orders": "طلبات الزبائن",
  "All orders": "كل الطلبيات",
  "New order": "إضافة طلب جديد",
  "Add order": "إضافة طلب جديد",
  "New order title": "طلب جديد",
  "Add the order details and create a new order": "أضف تفاصيل الطلب وإنشاء طلبية جديدة",
  "Save order": "حفظ الطلب",
  "Customer information": "معلومات العميل",
  "Customer info": "معلومات العميل",
  "Delivery address": "عنوان التوصيل",
  "Order summary": "ملخص الطلب",
  "Summary": "الملخص",
  "More info": "معلومات إيضافيه",
  "Address description": "الوصف",
  "Street, area, landmark, house number…": "الشارع، المنطقة، علامة مميزة، رقم المنزل…",
  "Delivery fee": "رسوم التوصيل",
  "Total": "الإجمالي",
  "Total with delivery fee": "الإجمالي بالتوصيل",
  "Total delivery and shipping": "الإجمالي بالتوصيل و الشحن",
  "Total with shipping fee": "الإجمالي بالشحن",
  "Rejected": "مرفوض",
  "Customer details": "بيانات الزبون والتوصيل",
  "Customer & delivery": "بيانات الزبون والتوصيل",
  "WhatsApp or additional phone number": "واتساب او رقم هاتف إضافي",
  "WhatsApp": "واتساب",
  "Require Additional Phone Number from customer": "طلب رقم هاتف إضافي من الزبون",
  "Address": "العنوان بالكامل",
  "Full delivery address": "العنوان بالكامل",
  "Delivery location": "مكان التوصيل",
  "Delivery zones": "مناطق التوصيل",
  "Shipping cost": "تكلفة الشحن",
  "ships to": "يشحن إلى",
  "Ships to": "يشحن إلى",
  "Country": "الدولة",
  "City": "المدينة",
  "Any city": "أي مدينة",
  "Notes": "ملاحظات",
  "Marketer note:": "ملاحظات المسوق:",
  "Special instructions, preferred delivery time, etc.": "تعليمات خاصة، وقت التوصيل المفضل، إلخ.",
  "Order total": "إجمالي الطلبية",
  "Total to pay": "الإجمالي المطلوب دفعه",
  "Confirm order": "تأكيد الطلبية",
  "Save changes": "حفظ التغييرات",
  "Save changes (Draft)": "حفظ التغييرات (مسودة)",
  "Edit order": "تعديل الطلبية",
  "Unknown shop": "متجر غير معروف",
  "No earnings yet": "لا توجد أرباح بعد",
  "To ship": "بانتظار الشحن",
  "Shipped": "تم الشحن",
  "Order delivered": "تم تسليم الطلبية",
  "Order failed": "فشلت الطلبية",
  "Mark shipped": "تعيين كـ تم الشحن",
  "Mark delivered": "تعيين كـ تم التسليم",
  "Ship": "شحن",
  "Order ID": "رقم الطلبية",
  "New": "جديد",
  "New orders today": "طلبيات جديدة اليوم",
  "Leads Today": "العملاء المحتملون اليوم",
  "Confirmed": "تم التأكيد",
  "awaiting": "في الانتظار",
  "completed": "مكتملة",
  
  "Send order to business owner": "إرسال الطلبية لصاحب العمل",
  "Forwarded to Business": "تم التحويل للتاجر",
  "Forwarded to business": "تم التحويل للتاجر",
  "Draft": "مسودة",
  "Not sent": "لم يتم الإرسال",
  "Reviewed": "تمت المراجعة",
  "Approved": "تمت الموافقة",

  // ===== Commitment Fee & Payments =====
  "COMMITMENT FEE TO COLLECT": "قيمة العربون المطلوب",
  "TOTAL COMMITMENT FEE FROM CUSTOMER": "إجمالي العربون المطلوب من الزبون",
  "Fee to collect": "العربون المطلوب",
  "Fee pending": "العربون معلق",
  "Fee/unit": "العربون للقطعة",
  "per unit · calculated by Wasla": "للقطعة · محسوبة بواسطة وصلة",
  "Collect this from the customer before shipping. Set by Wasla.": "يتم تجميعه من الزبون قبل الشحن. محدد بواسطة وصلة.",
  "Mandatory Upfront Deposit": "العربون المطلوب (إجباري)",
  "An upfront payment is required before this order can be processed.": "لازم تدفع العربون قبل ما يتم إرسال الطلب.",
  
  "Your Marketer Fee:": "عمولتك كمسوّق:",
  "Your Marketer Fee": "عمولتك كمسوّق",
  "Platform Security Fee:": "رسوم المنصة:",
  "Platform Security Fee": "رسوم المنصة",
  "Total:": "الإجمالي:",
  "Total fee required": "إجمالي العربون المطلوب",
  "of the product price.": "من سعر المنتج.",
  "Explain that a small commitment fee is required before their order is placed. This reserves the product and confirms they are serious.": "اشرح للزبون إن فيه عربون بسيط مطلوب قبل تأكيد الطلب لضمان الجدية.",
  "Did the customer pay the commitment fee?": "هل دفع الزبون عربون الحجز؟",
  "Yes, paid": "نعم، تم الدفع",
  "No, not yet": "لا، مزال",
  "Customer pays remaining balance on arrival": "الزبون يدفع باقي المبلغ عند الاستلام",
  "Cash on delivery": "الدفع عند الاستلام",
  "This fee is non-refundable if the customer cancels. It protects you and the business from fake orders.": "هذا العربون غير قابل للترجيع لو الزبون لغى الطلبية، عشان يحميك ويحمي تاجر من الطلبات الوهمية.",
  "How to collect the fee": "كيف تحصل العربون",
  "How to collect fee": "كيف تحصل الرسوم",
  "Request a deposit from the customer": "اطلب عربون من الزبون",
  "Ask the customer to deposit": "اطلب من الزبون يودّع",
  "into this account.": "في هذا الحساب.",
  "Send the payment receipt": "أرسل إيصال الدفع",
  "Once the deposit is made, upload a photo of the receipt so it can be reviewed.": "بعد إتمام التحويل، ارفع صورة الإيصال ليتم مراجعته.",
  "The company will verify the deposit, then credit your funds to your wallet for withdrawal.": "ستقوم الشركة بالتحقق من التحويل، ثم تُضاف أموالك إلى محفظتك للسحب.",
  "Your fee": "رسومك",
  "Total deposit required": "إجمالي العربون المطلوب",
  "Important notes": "ملاحظات مهمة",
  "Step-by-step deposit instructions": "خطوات التحويل بالتفصيل",
  "COMMITMENT FEE — COLLECT FROM CUSTOMER": "عربون الحجز — احصله من الزبون",
  "Tell the customer the fee": "خبر الزبون بقيمة العربون",
  "Share your payment details": "شارك بيانات حسابك",
  "Payment details": "بيانات الدفع",
  "Ask the customer to send the fee to:": "اطلب من الزبون يبعت العربون لـ:",
  "Bank": "المصرف",
  "Account name": "اسم الحساب",
  "Account holder": "اسم الحساب",
  "Account number": "رقم الحساب",
  "IBAN": "رقم الآيبان",
  "Mobile wallet": "محفظة الموبايل",
  "PayPal": "باي بال",
  "Affiliate link": "رابط الإحالة",
  "Ask for a screenshot": "اطلب سكرين شوت",
  "Once sent, ask the customer to share a screenshot of the bank transfer confirmation.": "بعد ما يبعت، اطلب منه سكرين شوت لتأكيد التحويل.",
  "Upload and confirm": "ارفع السكرين وأكد",
  "Upload the screenshot in the order form and tap 'Yes, paid.' The order is then sent to the business for processing and shipping.": "ارفع السكرين شوت في فورم الطلبية واضغط 'نعم، تم الدفع'. الطلبية حتمشي للتاجر للتجهيز والشحن.",
  "Payment receipt": "إيصال الدفع",
  "Tap to upload receipt": "اضغط لرفع الإيصال",
  "Upload payment receipt": "رفع إيصال الدفع",
  "Bank transfer": "تحويل بنكي",
  "Choose where the receipt is coming from": "اختر مصدر الإيصال",
  "Full name on account": "الاسم الكامل لصاحب الحساب",
  "optional": "اختياري",
  "(optional)": "(اختياري)",
  "Marketer confirmed payment": "المسوّق أكد الدفع",
  "View payment receipts": "عرض إيصالات الدفع",
  "Upfront fee paid": "تم دفع العربون",
  "Add receipt and send": "إضافة الإيصال والإرسال",
  "Receipt uploaded": "تم رفع الإيصال",
  "!Important": "!هام",
  "About the upfront deposit": "عن العربون المطلوب",
  "Your order will not be sent to the business owner until you upload proof of the upfront payment.": "الطلب ما يوصلش لصاحب النشاط إلا بعد رفع إيصال دفع العربون.",
  "Once your payment is reviewed and approved, the amount will appear in your Wallet as secured funds.": "بعد مراجعة الإيصال والموافقة عليه، قيمة العربون تنزل في محفظتك كرصيد مضمون يمكن سحبه.",
  "This system protects the rights and responsibilities of both marketers and business owners.": "النظام هذا يحفظ حقوق والتزامات المسوّق وصاحب النشاط.",
  "Payments made outside the platform are not protected, and the platform accepts no responsibility for any fraud for off-platform transactions.": "أي دفعات تتم خارج المنصة غير محميه، و تكون على مسؤولية أصحابها، والمنصة غير مسؤولة عنها في حالات النصب و الإحتيال بأي شكل.",
  "The amount shown is calculated per unit. If you increase the quantity, the required upfront payment increases accordingly.": "القيمة المعروضة تخص قطعة واحدة. وإذا زدت الكمية، قيمة العربون تزيد بنفس النسبة.",
  "After completing the payment, upload your payment receipt to continue.": "بعد إتمام التحويل، ارفع إيصال الدفع حتى يتم متابعة الطلب.",
  "The fee is deducted from the product price, and is not an additional charge.": "قيمة العربون تُخصم من سعر المنتج، وليست مبلغاً إضافياً على سعر المنتج.",
  "Amount customer pays on Delivery:": "المبلغ الذي يدفعه الزبون عند الإستلام:",
  "Delivery cost:": "تكلفة التوصيل:",
  "Shipping cost:": "تكلفة الشحن:",

  // ===== Empty States & Search =====
  "No orders found.": "لم يتم العثور على طلبيات.",
  "No orders yet.": "لا توجد طلبيات حتى الآن.",
  "Tap the button below to add your first order.": "اضغط على الزر تحت لإضافة أول طلبية.",
  "Tap the button above to add your first order.": "اضغط على الزر أعلاه لإضافة أول طلبية.",
  "No products found.": "لم يتم العثور على منتجات.",
  "No products yet.": "لا توجد منتجات حتى الآن.",
  "No saved products yet.": "لا توجد منتجات محفوظة بعد.",
  "Browse and tap the heart to save products you want to market.": "تصفح واضغط على القلب لحفظ المنتجات اللي تبي تسوقلها.",
  "No cities yet": "لا توجد مدن مضافة بعد",
  "No notifications yet.": "لا توجد إشعارات بعد.",
  "Search products…": "ابحث عن منتجات...",
  "Search your orders": "ابحث عن طلبات",
  "Search your favorite products": "ابحث عن منتجاتك المفضله",
  "Payout / Bank details": "تفاصيل السحب / البنك",
  "Search currencies…": "ابحث عن العملات...",
  "Search countries…": "ابحث عن الدول...",
  "Search by name, ID, city, product…": "ابحث بالاسم، رقم الطلبية، المدينة، المنتج...",
  "Search by name, number, role, email…": "ابحث بالاسم أو الرقم أو الدور أو البريد…",
  "Search by name, phone, business…": "ابحث بالاسم أو الهاتف أو تاجر…",
  "Search products & marketers…": "ابحث عن المنتجات والمسوّقين…",
  "Search orders (customer, product, city, ID)": "ابحث في الطلبات (العميل، المنتج، المدينة، رقم الطلب)",
  "Search orders…": "ابحث عن الطلبات",
  "Search products (name, code, city)": "ابحث في المنتجات (الاسم، الكود، المدينة)",
  "Street, area, landmark…": "الشارع، المنطقة، علامة مميزة…",
  "Brief description for marketers…": "وصف موجز للمسوّقين…",
  "e.g. product name": "مثال: اسم المنتج",
  "e.g. notes": "مثال: ملاحظاتك هنا",

  // ===== Misc UI =====
  "Verify": "التحقق",
  "Welcome": "مرحباً",
  "Loading…": "جارٍ التحميل…",
  "Try again": "إعادة المحاولة",
  "Go home": "الذهاب للرئيسية",
  "Page not found": "الصفحة غير موجودة",

  // ===== Public product share page (/p/:id, non-registered visitors) =====
  "Product unavailable": "المنتج غير متاح",
  "This product is no longer available.": "هذا المنتج لم يعد متاحاً.",
  "Code:": "الكود:",
  "Share": "مشاركة",
  "Delivery to": "التوصيل إلى",
  "No delivery zones": "لا توجد مناطق توصيل",
  "No cities": "لا توجد مدن",
  "Shipping:": "الشحن:",
  "Delivery:": "التوصيل:",
  "Reviews (": "التقييمات (",
  "No reviews yet.": "لا توجد تقييمات بعد.",
  "· out of stock": "· نفذت الكميه",
  "Sell this product": "بيع هذا المنتج",
  "Open dashboard": "فتح لوحة التحكم",
  "Create an account to sell the product": "إنشاء حساب لبيع المنتج",

  // ===== Admin =====
  // ----- Admin: employees & payroll -----
  "Pay day": "يوم الدفع",
  "Salary": "الراتب",
  "Phone numbers": "أرقام الهاتف",
  "Phone number 1": "رقم الهاتف 1",
  "Phone number 2": "رقم الهاتف 2",
  "Employment date": "تاريخ التعيين",
  "Notes (optional)": "ملاحظات (اختياري)",
  "Monthly salary": "الراتب الشهري",
  "Paid This Cycle": "المدفوع هذي الدورة",
  "Pending This Cycle": "قيد الانتظار هذي الدورة",
  "Mark as Paid": "سجّلها مدفوعة",
  "Paid ✓": "مدفوع ✓",
  "No employees match.": "ما فيش موظفين مطابقين.",
  "Total paid": "إجمالي المدفوع",
  "Payment": "الدفعة",
  "Payments": "الدفعات",
  "Paid at": "تاريخ الدفع",
  "No payments recorded yet.": "ما تسجلت حتى دفعة لتوه.",
  "Full name is required.": "الاسم الكامل مطلوب.",
  "Employee number is required.": "رقم الموظف مطلوب.",
  "Job title is required.": "المسمى الوظيفي مطلوب.",
  "Email is required.": "الإيميل مطلوب.",
  "That email does not look right.": "الإيميل هذا شكله غالط.",
  "Phone number 1 is required.": "رقم الهاتف 1 مطلوب.",
  "Phone number 2 is required.": "رقم الهاتف 2 مطلوب.",
  "The two phone numbers have to be different.": "لازم الرقمين يكونوا مختلفين.",
  "Monthly salary is required.": "الراتب الشهري مطلوب.",
  "Monthly salary has to be a number.": "الراتب الشهري لازم يكون رقم.",

  // ----- Admin: shared chrome -----
  "Search": "بحث",
  "Search by name, email, phone, business…": "دوّر بالاسم أو الإيميل أو الهاتف أو الشركة…",
  "Search by name, code, shop…": "دوّر بالاسم أو الكود أو المحل…",
  "Search by marketer name, phone, or email": "دوّر باسم المسوّق أو الهاتف أو الإيميل",
  "Search by product, customer name, phone, or order code": "دوّر بالمنتج أو اسم الزبون أو الهاتف أو كود الطلبية",
  "Language": "اللغة",
  "Status": "الحالة",
  "Banned": "محظور",
  "Frozen": "مجمّد",
  "Date joined": "تاريخ الانضمام",
  "Type WIPE to confirm": "اكتب WIPE للتأكيد",
  "Notification title (what they see first)": "عنوان الإشعار (اللي يشوفوه أول)",
  "Notification title (what marketers see first)": "عنوان الإشعار (اللي يشوفوه المسوّقين أول)",
  "Notification content (shown when tapped)": "محتوى الإشعار (يبان كي يضغطوا عليه)",
  "Full message…": "الرسالة كاملة…",
  "e.g. Your account was reviewed": "مثال: تمت مراجعة حسابك",
  "e.g. New update available": "مثال: فمّا تحديث جديد",

  // ----- Admin: menu & pages -----
  "Wallet balance": "رصيد المحفظة",
  "Scheduled for": "مجدولة ليوم",
  "No requests here.": "ما فيش طلبات هنا.",
  "No requests match your search.": "ما فيش طلبات مطابقة لبحثك.",
  "Send to All Users": "ابعت لكل المستخدمين",
  "Broadcast to all users": "إرسال لكل المستخدمين",
  "Notification title (what users see first)": "عنوان الإشعار (اللي يشوفوه المستخدمين أول)",
  "No reports match your search.": "ما فيش بلاغات مطابقة لبحثك.",
  "Report Type": "نوع البلاغ",
  "Comment": "التعليق",
  "Phone Number": "رقم الهاتف",
  "Product Code": "كود المنتج",
  "Go to Account": "افتح الحساب",
  "Open product details": "افتح تفاصيل المنتج",
  "No open reports.": "ما فيش بلاغات مفتوحة.",
  "Feedback": "ملاحظات",
  "Send note to business": "ابعت ملاحظة للتاجر",
  "Write a note to the business about this report": "اكتب ملاحظة للتاجر بخصوص هذا البلاغ",
  "Accounts asking to be removed": "حسابات طالبة الحذف",
  "Products flagged by marketers": "منتجات بلّغ عليها المسوّقون",
  "Staff, pay cycles and history": "الموظفون ودورات الدفع والسجل",
  "Broadcast to all marketers": "إرسال لكل المسوّقين",
  "Approve & schedule deletion": "الموافقة وجدولة الحذف",
  "Reject": "رفض",
  "no phone": "بلا رقم هاتف",
  "no email": "بلا إيميل",
  "Go to marketer account": "افتح حساب المسوّق",
  "Product no longer available": "المنتج ما عادش متوفر",
  "Send review to marketer": "ابعت المراجعة للمسوّق",
  "Attach photo (optional)": "أرفق صورة (اختياري)",
  "Send to All Marketers": "ابعت لكل المسوّقين",
  "Reports": "البلاغات",
  "Send Notification": "إرسال إشعار",
  "Deletion Requests": "طلبات حذف الحساب",
  "Requests & Reports": "الطلبات والبلاغات",
  "Deletions": "طلبات الحذف",
  "Needs Review": "تحتاج مراجعة",
  "Scheduled": "مجدولة",
  "Cancelled": "ملغاة",
  "Resolved": "تمت معالجتها",
  "Wallet is empty.": "المحفظة فاضية.",
  "Admin comment:": "تعليق الأدمن:",
  "Scheduled for:": "مجدولة ليوم:",
  "· Pending:": "· قيد الانتظار:",
  "Optional note for approval, or the reason if you're rejecting this request": "ملاحظة اختيارية للموافقة، ولا السبب إذا كنت ترفض الطلب",
  "Optional note for approval, or the reason if you\u2019re rejecting this request": "ملاحظة اختيارية للموافقة، ولا السبب إذا كنت ترفض الطلب",
  "Report type": "نوع البلاغ",
  "Marketer\u2019s comment": "تعليق المسوّق",
  "Marketer's comment": "تعليق المسوّق",
  "No comment": "بلا تعليق",
  "Write your review of this report — the marketer will see it as 'Report reviewed'": "اكتب مراجعتك للبلاغ — المسوّق بيشوفها كـ \u2018تمت مراجعة البلاغ\u2019",

  // ----- Admin: verify & payouts -----
  "No payouts match your search.": "ما فيش سحوبات مطابقة لبحثك.",
  "Account #": "رقم الحساب",
  "Attach transfer receipt": "أرفق إيصال التحويل",
  "Receipt attached": "تم إرفاق الإيصال",
  "Attach the transfer receipt first": "أرفق إيصال التحويل أول",
  "Paid": "مدفوع",
  "Order details": "تفاصيل الطلبية",
  "Order value": "قيمة الطلبية",
  "Customer": "الزبون",
  "Created": "تم الإنشاء",
  "Uploaded": "تم الرفع",
  "Refunded": "تم الإرجاع",
  "All clear": "كل شيء تمام",
  "Admin note": "ملاحظة الأدمن",
  "Marketer fee": "رسوم المسوّق",
  "Refund customer": "رجّع فلوس الزبون",
  "Refund this order": "رجّع فلوس هذي الطلبية",
  "Explain why this order is being refunded…": "وضّح علاش تم إرجاع فلوس هذي الطلبية…",
  "Send a note to the marketer (e.g. missing IBAN)": "ابعت ملاحظة للمسوّق (مثال: الآيبان ناقص)",
  "This marketer has no receipts to show.": "المسوّق هذا ما عندوش إيصالات نعرضوها.",
  "✓ Approved": "✓ تمت الموافقة",
  "✕ Rejected": "✕ مرفوض",
  "✕ Failed": "✕ فشلت",
  "↺ Refunded": "↺ تم الإرجاع",

  // ----- Admin: home & product sheet -----
  "Net profit": "صافي الربح",
  "Live Users": "المستخدمون المتصلون الآن",
  "Peak Live Users": "أعلى عدد متصلين",
  "Total Products": "إجمالي المنتجات",
  "Platform Fees": "رسوم المنصة",
  "Performance over all time": "الأداء عبر كل الوقت",
  "Performance": "الأداء",
  "Succeeded Upfronts": "العرابين الناجحة",
  "Succeeded Pieces Sold": "القطع المبيوعة الناجحة",
  "Delivered orders only, lifetime": "الطلبيات الموصّلة بس، من البداية",
  "Receipts approved, lifetime": "الإيصالات المقبولة، من البداية",
  "Active marketers": "المسوّقون النشطون",
  "Marketer\u2019s earning per sale": "ربح المسوّق من كل عملية بيع",
  "Marketer's earning per sale": "ربح المسوّق من كل عملية بيع",
  "Deposit (with platform fee)": "العربون (مع رسوم المنصة)",
  "Product code:": "كود المنتج:",
  "Business owner name:": "اسم صاحب العمل:",
  "Email:": "الإيميل:",

  "Admin": "مدير",
  "Admins": "المديرون",
  "Admin Console": "لوحة تحكم المدير",
  "Wasla platform control": "إدارة منصة وصلة",
  "Users": "المستخدمون",
  "Total Users": "إجمالي المستخدمين",
  "Active Users": "المستخدمون النشطون",
  "All registered accounts": "كل الحسابات المسجّلة",
  "User Directory": "دليل المستخدمين",
  "Businesses": "الشركات",
  "Employees": "الموظفون",
  "New Employee": "موظف جديد",
  "Employees & Payroll": "الموظفون والرواتب",
  "Employee number": "رقم الموظف",
  "Job title": "المسمى الوظيفي",
  "Date of hire": "تاريخ التعيين",
  "Delete employee": "حذف الموظف",
  "Total Monthly Salaries": "إجمالي الرواتب الشهرية",
  "Total Platform Fees Collected": "إجمالي رسوم المنصة المحصّلة",
  "Paid This Month": "المدفوع هذا الشهر",
  "Pending This Month": "قيد الانتظار هذا الشهر",
  "Global Analytics": "تحليلات شاملة",
  "Payout Manager": "إدارة السحوبات",
  "Order Verification Hub": "مركز التحقق من الطلبيات",
  "Product Review": "مراجعة المنتجات",
  "Select a country": "اختر الدولة",
  "All-time": "كل الأوقات",
  "all time": "كل الوقت",
  "All time": "منذ إنشاء الحساب",
  "Day": "اليوم",
  "Month": "الشهر",
  "Year": "السنه",
  "Clear selection": "مسح التحديد",
  "Analytics": "الإحصائيات",
  "Breakdown": "الإحصائيات",
  "Transactions": "التحركات",
  "No transactions yet.": "لا توجد تحركات بعد.",

  // ===== Entities (lowercase plurals) =====
  "products": "منتجات",
  "orders": "طلبيات",
  "cities": "مدن",
  "days": "أيام",
  "day": "يوم",

  // ===== Chart periods & dates =====
  "D": "يوم",
  "W": "أ",
  "M": "شهر",
  "Y": "سنة",
  "Business workspace": "مساحة عمل تاجر",
  "Marketer workspace": "مساحة عمل المسوّق",
  "Mon": "الإثنين", "Tue": "الثلاثاء", "Wed": "الأربعاء",
  "Thu": "الخميس", "Thur": "الخميس", "Fri": "الجمعة", "Sat": "السبت", "Sun": "الأحد",
  "Jan": "يناير", "Feb": "فبراير", "Mar": "مارس", "Apr": "أبريل",
  "May": "مايو", "Jun": "يونيو", "Jul": "يوليو", "Aug": "أغسطس",
  "Sep": "سبتمبر", "Oct": "أكتوبر", "Nov": "نوفمبر", "Dec": "ديسمبر",

  // ===== Final sweep: actions, selects, uploads =====
  "- Select -": "- اختار -",
  "— Select a product —": "— اختار منتج —",
  "-- Select --": "- اختار -",
  "Add value +": "+ إضافة قيمة",
  "Add another variant +": "+ إضافة خيار تاني",
  "Add country to zones +": "+ إضافة دولة لمناطق التوصيل",
  "Take photo (Camera)": "التقاط صورة (الكاميرا)",
  "Choose from Gallery": "اختيار من الاستوديو",
  "Upload from Files": "رفع من الملفات",
  "Add payment receipt": "إضافة إيصال الدفع",
  "Use your camera": "استخدم الكاميرا",
  "Pick an existing photo": "اختر صورة من هاتفك",
  "Photos, PDF, or other files": "صور أو PDF أو ملفات أخرى",
  "Save as Draft (keep editing later)": "حفظ كمسودة (الاستمرار في التعديل لاحقاً)",

  // ===== Order details / receipts =====
  "ORDER": "الطلبية",
  "Order": "الطلبية",
  "Code": "الكود",
  "Shipping": "الشحن",
  "Approved · forwarded to business ✓ ✓": "تمت الموافقة · تم التحويل للتاجر ✓ ✓",
  "Pending verification by admin ⏳": "في انتظار مراجعة الإدارة ⏳",
  "View payment receipt": "عرض إيصال الدفع",
  "commission": "العمولة",

  // ===== Product creation placeholders =====
  "e.g. Running shoes": "مثال: أحذية رياضية",
  "Brief description for marketers...": "...وصف مختصر للمسوّقين",
  "...Brief description for marketers": "...وصف مختصر للمسوّقين",
  "Special instructions, preferred delivery time, etc": "ملاحظات خاصة، وقت التوصيل المفضل، إلخ",
  ".Special instructions, preferred delivery time, etc": "ملاحظات خاصة، وقت التوصيل المفضل، إلخ.",
  "Sizes and Colours are added by default. Rename, remove, or add your own (e.g. Material, Flavour, Style)":
    "المقاسات والألوان مضافة افتراضياً. تقدر تغير اسمها، تمسحها، أو تضيف خياراتك الخاصة (مثال: الخامة، النكهة، الستايل)",
  "Add countries and cities. Set shipping and delivery cost per city.":
    "ضيف الدول والمدن. وحدد تكلفة الشحن والتوصيل لكل مدينة.",

  // ===== Final sweep — missing user-facing strings =====
  "— Add in profile —": "— أضف في الملف الشخصي —",
  "— Select —": "— اختر —",
  "— Select city —": "— اختر المدينة —",
  "— Select country —": "— اختر الدولة —",
  "— Select country first —": "— اختر الدولة أولاً —",
  "Select…": "اختر…",
  "+ Add another variant": "+ أضف خياراً آخر",
  "+ Add country to zones": "+ أضف دولة إلى المناطق",
  "+ Add value": "+ أضف قيمة",
  "+ New": "+ جديد",
  "Add a city…": "أضف مدينة…",
  "⏳ Pending verification": "⏳ بانتظار التحقق",
  "⏳ Pending verification by admin": "⏳ بانتظار تحقق الإدارة",
  "✏ Edit": "✏ تعديل",
  "✕ Receipt rejected": "✕ تم رفض الإيصال",
  "🗑 Delete": "🗑 حذف",
  "📷 Take photo (Camera)": "📷 التقط صورة (الكاميرا)",
  "🖼️ Choose from Gallery": "🖼️ اختر من المعرض",
  "📁 Upload from Files": "📁 ارفع من الملفات",
  "📝 Draft not sent": "📝 مسودة غير مرسلة",
  "Add receipt & send": "أضف الإيصال وأرسل",
  "Re-upload receipt": "أعد رفع الإيصال",
  "Receipt will go to admin for verification, then forwarded to the business owner.":
    "سيتم إرسال الإيصال للإدارة للتحقق، ثم يُحوَّل لصاحب العمل.",
  "Saved locally. Upload a receipt to send for verification.":
    "تم الحفظ محلياً. ارفع الإيصال لإرساله للتحقق.",
  "Analysing image…": "جاري تحليل الصورة…",
  "Any": "الكل",
  "Approve & forward": "اعتماد وتحويل",
  "Approved forwarded to business": "تمت الموافقة وتم التحويل للتاجر",
  "Awaiting business": "بانتظار تاجر",
  "Bank / Provider": "البنك / المزود",
  "Camera Gallery Files": "الكاميرا · المعرض · الملفات",
  "Cash / Other": "نقداً / أخرى",
  "Confirmed & delivered, selected month": "تم التأكيد والتوصيل، الشهر المحدد",
  "Confirmed & delivered, selected year": "تم التأكيد والتوصيل، السنة المحددة",
  "Contact name:": "اسم جهة الاتصال:",
  "e.g. Barclays": "مثال: باركليز",
  "e.g. Omar Hassan": "مثال: عمر حسن",
  "Failed to load.": "فشل التحميل.",
  "Fees in": "الرسوم في",
  "Funds arrive in 2–5 business days": "تصل الأموال خلال 2–5 أيام عمل",
  "Hidden": "مخفي",
  "Hired": "تم التوظيف",
  "History": "السجل",
  "Joined:": "انضم في:",
  "Last 30 days": "آخر 30 يوماً",
  "Method": "الطريقة",
  "Monthly salary (£)": "الراتب الشهري (£)",

  // ===== Final sweep additions =====
  "· Deliv": "· التوصيل",
  "· commission · cities": "· العمولة · المدن",
  "⚠ Admin:": "⚠ المسؤول:",
  "✓ Approved · forwarded to business": "✓ تمت الموافقة · تم التحويل للتاجر",
  "✓ Receipt already uploaded": "✓ تم رفع الإيصال مسبقاً",
  "✓ Receipt ready ·": "✓ الإيصال جاهز ·",
  "📝 Draft · not sent": "📝 مسودة · لم تُرسل",
  "across payment": "عبر الدفعات",
  "Action failed:": "فشلت العملية:",
  "all-time": "منذ إنشاء الحساب",
  "Approve failed:": "فشلت الموافقة:",
  "Approve this receipt? The order will be forwarded to the business owner and stock will be decremented.":
    "هل تريد الموافقة على هذا الإيصال؟ سيتم تحويل الطلبية لصاحب العمل وخصم المخزون.",
  "Camera · Gallery · Files": "كاميرا · معرض · ملفات",
  "Code: ·": "الكود: ·",
  "Confirm receipt of": "تأكيد استلام",
  "Confirm you have manually transferred": "أكد أنك قمت بالتحويل يدوياً",
  "Could not save order:": "تعذّر حفظ الطلبية:",
  "Could not save product:": "تعذّر حفظ المنتج:",
  "Delete failed:": "فشل الحذف:",
  "Delete this employee and their payment history? This cannot be undone.":
    "هل تريد حذف هذا الموظف وسجل مدفوعاته؟ لا يمكن التراجع عن هذا الإجراء.",
  "Delete this product? Marketers will no longer see it.":
    "هل تريد حذف هذا المنتج؟ لن يظهر للمسوّقين بعد الآن.",
  "e.g. 12": "مثال: 12",
  "employees ·": "موظفون ·",
  "Failed to delete": "فشل الحذف",
  "Failed to update status": "فشل تحديث الحالة",
  "For": "لـ",
  "Found:": "النتائج:",
  "Includes your commission of": "تشمل عمولتك بقيمة",
  "Mark": "تحديد",
  "Name and employee number are required.": "الاسم ورقم الموظف مطلوبان.",
  "Open": "فتح",
  "paid this month": "المدفوع هذا الشهر",
  "Photo upload failed.": "فشل رفع الصورة.",
  "Please add at least one delivery zone.": "أضف منطقة توصيل واحدة على الأقل.",
  "Please complete at least one city.": "أكمل بيانات مدينة واحدة على الأقل.",
  "Please fill in name, price and commission.": "يرجى إدخال الاسم والسعر والعمولة.",
  "Please fill in your payout method, bank, account holder, and account number so the admin can pay you.":
    "يرجى تعبئة طريقة السحب، البنك، اسم صاحب الحساب، ورقم الحساب حتى يتمكن المسؤول من الدفع لك.",
  "Please select a currency.": "يرجى اختيار عملة.",
  "Reason for rejecting this receipt? (visible to the marketer)":
    "سبب رفض هذا الإيصال؟ (سيظهر للمسوّق)",
  "Reject failed:": "فشل الرفض:",
  "Save failed:": "فشل الحفظ:",
  "Send Order to Business Owner": "إرسال الطلبية إلى صاحب العمل",
  "Status:": "الحالة:",
  "Tap to open": "اضغط للفتح",
  "This product is not available for delivery to": "هذا المنتج غير متوفر للتوصيل إلى",
  "To send for verification: please fill customer name, phone, product, country and city.":
    "للإرسال للتحقق: يرجى تعبئة اسم الزبون، رقم الهاتف، المنتج، الدولة والمدينة.",
  "Upload failed — tap to try again (": "فشل الرفع — اضغط للمحاولة مجدداً (",
  "Upload service not ready — refresh and try again":
    "خدمة الرفع غير جاهزة — حدّث الصفحة وأعد المحاولة",
  "No matches": "لا توجد نتائج",
  "No payout details on file — contact the marketer.":
    "لا توجد بيانات صرف مسجلة — تواصل مع المسوّق.",
  "No payout requests pending.": "لا توجد طلبات صرف معلقة.",
  "No receipt image": "لا توجد صورة للإيصال",
  "No receipts awaiting review.": "لا توجد إيصالات بانتظار المراجعة.",
  "No users found.": "لم يتم العثور على مستخدمين.",
  "No variants. Tap \"Add another variant\" below.":
    "لا توجد خيارات. اضغط \"أضف خياراً آخر\" بالأسفل.",
  "Not found.": "غير موجود.",
  "Payday": "يوم الدفع",
  "per unit calculated by Wasla": "للوحدة — تحسبها وصلة",
  "Platform fee (5%)": "رسوم المنصة (5%)",
  "Platform fee (per unit)": "رسوم المنصة (للوحدة)",
  "Product not found.": "المنتج غير موجود.",
  "Receipt": "الإيصال",
  "Reject with note": "رفض مع ملاحظة",
  "Remove group": "حذف المجموعة",
  "Save & request withdrawal": "حفظ وطلب السحب",
  "SWIFT/BIC": "SWIFT/BIC",
  "Total paid:": "إجمالي المدفوع:",
  "Track & manage orders": "تتبع وإدارة الطلبيات",
  "Upload the screenshot in the order form and tap \"Yes, paid.\" The order is then sent to the business for processing and shipping.":
    "ارفع الصورة في نموذج الطلبية واضغط \"نعم، تم الدفع\". بعدها يتم إرسال الطلبية للتاجر للمعالجة والشحن.",
  "Variant name": "اسم الخيار",
  "Your commission (per unit)": "عمولتك (للوحدة)",
  "EMP-001": "EMP-001",
  "receipt": "إيصال",
  "More soon": "المزيد قريباً",
  "One pay": "وان باي",
  "Bank of Unity": "مصرف الوحده",
  "Libyana Credit": "رصيد ليبيانا",
  "Madar Credit": "رصيد مدار",
  "Wahda bank": "مصرف الوحده",
  "Support": "مساعدة",
  "Have a problem?": "عندك مشكله؟",
  "Contact admin": "راسل الأدمن",
  "This system protects the rights and responsibilities of both marketers and business owners. Upfront fees are non-refundable under all circumstances. Deposits cannot be reversed once made, except if the customer does not receive the product. In that case, contact the admin:":
    "النظام هذا يحفظ حقوق والتزامات المسوق وصاحب النشاط. العربون غير قابل للاسترجاع في أي حال من الأحوال، والإيداع ما ينلغي إلا إذا الزبون ما استلمش المنتج. في هالحالة تواصل مع الإدارة:",
  "After verification, the upfront fee will be refunded and the business owner will be permanently banned. If the product has been delivered, any dispute is the business owner's responsibility. The admin may only hide or ban the business owner only.":
    "بعد التحقق يتم ترجيع العربون وحظر صاحب النشاط نهائياً. أما إذا الزبون استلم المنتج، فالمسؤولية على صاحب النشاط، وإجراء الإدارة يقتصر على إخفاء أو حظر الحساب فقط.",
  "Number starts with 092 or 094": "الرقم يبدا من 092 او 094",
  "Number starts with 091 or 093": "الرقم يبدا من 091 او 093",
  "Write any additional notes": "اكتب أي ملاحظات إضافية",

  // ===== Comprehensive audit additions =====
  "Alerts": "التنبيهات",
  "decrease": "تقليل",
  "increase": "زيادة",
  "Info": "معلومات",
  "Important info about upfront deposit": "معلومات مهمة عن العربون المسبق",
  "Setting up your account…": "جارٍ إعداد حسابك…",
  "If this persists, sign out and pick a role again.": "إذا استمرت المشكلة، سجّل الخروج واختر دورك من جديد.",
  "Edit Employee": "تعديل بيانات الموظف",
  "Enter": "أدخل",
  "Failed:": "فشل:",
  "Receipt rejected": "تم رفض الإيصال",
  "Kim Kardashian": "هيفاء وهبي",
  "Example: Kim Kardashian": "مثال: هيفاء وهبي",
  "Sunrise Kicks Ltd.": "شركة شروق الأحذية المحدودة",
  "Phone:": "الهاتف:",
  "Hired:": "تاريخ التوظيف:",
  "Payday:": "يوم الدفع:",
  "Ship:": "الشحن:",
  "Commission:": "العمولة:",
  "Platform fee:": "رسوم المنصة:",
  "Fee to collect:": "العربون المطلوب:",
  "LYD": "د.ل",
  "Product info": "بيانات المنتج",
  "Tap to upload photos": "انقر لتحميل صور المنتج",
  "First photo becomes the cover image": "الصورة الأولى هي صورة الغلاف",
  "This is what the marketer sees before tapping on the product to see the full photo": "هذه اللي يشوفه المسوق قبل النقر على المنتج عشان يشوف الصوره كامله",
  "What makes this product worth selling? Materials, fit, use case…": "ما الذي يجعل هذا المنتج يستحق البيع؟\nاوصف المنتج",
  "Select category": "اختر فئة المنتج",
  "Search category...": "ابحث عن الفئه",
  "Variants & inventory": "الخيارات والمخزون",
  "No variants": "بدون خيارات",
  "Has variants": "يحتوي على خيارات",
  "Countries and cities available for delivery": "الدول والمدن المتاحة للتوصيل",
  "+ Add a country": "+ إضافة دولة",
  "Marketer profit": "ارباح المسوق",
  "Percentage": "النسبه",
  "Set what marketers earn": "حدد مكسب المسوقين",
  "The higher the commission, the more marketers you'll attract to promote your product.": "كل ما زادت العمولة، زاد عدد المسوقين ليُرَوِّجو منتجاتك.",
  "LYD per sale": "د.ل لكل قطعه",
  "% of product price": "% من سعر المنتج",
  "Platform commission (5%)": "عمولة المنصه (5%)",
  "Your total": "الإجمالي",
  "Publish product": "أنشر المنتج",

  /* Admin console. These were the strings the console still showed in English
     on an Arabic page: the two report types the picker offers, the value that
     sits beside "الحالة" on a product, the label on the hide button once a
     product is already hidden, and the admins page. */
  "Merchant": "التاجر",
  "Other": "أخرى",
  "Hidden by admin": "مخفي من قبل الإدارة",
  "Paused by owner": "متوقف من قبل صاحبه",
  "Active": "نشط",
  "Unhide": "إظهار",
  "Add admin": "إضافة مسؤول",
  "Edit admin": "تعديل مسؤول",
  "No admins yet.": "لا يوجد مسؤولون بعد.",
  "All countries": "كل الدول",
  "Countries": "الدول",
  "Pages": "الصفحات",
  "Master": "المسؤول الرئيسي",
  "Invited": "تمت الدعوة",
  "Suspend": "إيقاف",
  "Restore": "استعادة",
  "Freeze": "تجميد",
  "Unfreeze": "إلغاء التجميد",
  "Unban": "إلغاء الحظر",
  "Ban Email": "حظر البريد",
  "No products match your search.": "لا توجد منتجات مطابقة لبحثك.",
  "No marketers match your search.": "لا يوجد مسوقون مطابقون لبحثك.",
};

// Build a case-insensitive lookup map (lowercased key -> Arabic value).
// First insertion wins, so the canonical-cased entries above take precedence
// for any future programmatic checks.
const DICT_CI: Record<string, string> = {};
for (const [k, v] of Object.entries(DICT)) {
  const lower = k.toLowerCase();
  if (!(lower in DICT_CI)) DICT_CI[lower] = v;
}

// ===== Pattern translations =====
// These handle dynamic strings with numbers/variables inside.
// Each pattern: regex + builder that receives the matched groups.
const AR_MONTHS: Record<string, string> = {
  Jan: "يناير", Feb: "فبراير", Mar: "مارس", Apr: "أبريل", May: "مايو", Jun: "يونيو",
  Jul: "يوليو", Aug: "أغسطس", Sep: "سبتمبر", Oct: "أكتوبر", Nov: "نوفمبر", Dec: "ديسمبر",
};
// Replaces English month abbreviations (Jan, Feb, ... Dec) inside a date string
// with their Arabic equivalents, leaving day/year digits untouched.
function arMonths(s: string): string {
  return s.replace(/\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b/g, (mo) => AR_MONTHS[mo] || mo);
}

type Pattern = { re: RegExp; build: (m: RegExpMatchArray) => string };
const PATTERNS: Pattern[] = [
  // "This account is registered as a business/marketer. Please use the ... sign-in page, or create a separate ... account."
  { re: /^This account is registered as a (business|marketer)\. Please use the \1 sign-in page, or create a separate (business|marketer) account\.?$/i, build: (m) => {
    const def: Record<string, string> = { business: "تاجر", marketer: "المسوق" };
    const indef: Record<string, string> = { business: "تاجر", marketer: "مسوق" };
    const actual = m[1].toLowerCase();
    const intent = m[2].toLowerCase();
    return `هذا الحساب مسجل كحساب ${def[actual]}. يرجى استخدام صفحة تسجيل الدخول الخاص ب${def[actual]}، أو إنشاء حساب ${indef[intent]} منفصل.`;
  } },
  // "Across 5 products" / "Across 12 cities" / "Across 3 orders"
  { re: /^Across\s+(\d[\d,]*)\s+products\b\.?$/i, build: (m) => `عبر ${m[1]} منتجات` },
  { re: /^Across\s+(\d[\d,]*)\s+cities\b\.?$/i, build: (m) => `عبر ${m[1]} مدن` },
  { re: /^Across\s+(\d[\d,]*)\s+orders\b\.?$/i, build: (m) => `عبر ${m[1]} طلبيات` },
  { re: /^Across\s+(\d[\d,]*)\s+shops?\b\.?$/i, build: (m) => `عبر ${m[1]} متاجر` },
  // "Order Code #1234" / "Order code: 1234"
  { re: /^Order\s+Code\s*[:#]?\s*(.+)$/i, build: (m) => `كود الطلبية ${m[1]}` },
  // "Product Code #1234"
  { re: /^Product\s+Code\s*[:#]?\s*(.+)$/i, build: (m) => `كود المنتج ${m[1]}` },
  // "Ships to <Country>"
  { re: /^Ships?\s+to\s+(.+)$/i, build: (m) => `يشحن إلى ${m[1]}` },
  // "Hey <Name>" / "Hey, <Name>"
  { re: /^Hey[,!]?\s+(.+)$/i, build: (m) => `مرحباً ${m[1]}` },
  // "Low stock: 3" / "Low stock (3)"
  { re: /^Low\s+stock\b(.*)$/i, build: (m) => `المخزون منخفض${m[1]}` },

  // "New (2)" -> "جديد (2)"
  { re: /^New\s*\((\d+)\)$/i, build: (m) => `جديد (${m[1]})` },
  // "Qty: 1" / "QTY 1" -> "الكمية: 1"
  { re: /^Qty\s*[:.]?\s*(\d[\d,]*)$/i, build: (m) => `الكمية: ${m[1]}` },
  { re: /^Quantity\s*[:.]?\s*(\d[\d,]*)$/i, build: (m) => `الكمية: ${m[1]}` },
  // "Shipping (Benghazi)" — translate inner city if dict has it
  { re: /^Shipping\s*\((.*)\)$/i, build: (m) => {
      const inner = m[1] ? (DICT[m[1]] || DICT_CI[m[1].toLowerCase()] || m[1]) : "";
      return `الشحن (${inner})`;
    } },
  // "per unit · marketer £5.00 + platform £1.00"
  { re: /^per\s+unit\s*[·\-]\s*marketer\s+(.+?)\s*\+\s*platform\s+(.+)$/i,
    build: (m) => `للوحدة · المسوّق ${m[1]} + المنصة ${m[2]}` },
  // "Delivery (City)"
  { re: /^Delivery\s*\((.+)\)$/i, build: (m) => {
      const inner = DICT[m[1]] || DICT_CI[m[1].toLowerCase()] || m[1];
      return `التوصيل (${inner})`;
    } },
  // "Receipt uploaded: <datetime>"
  { re: /^Receipt\s+uploaded\s*[:\-]\s*(.+)$/i, build: (m) => `تم رفع الإيصال: ${arMonths(m[1])}` },
  // "Reviewed: <datetime>"
  { re: /^Reviewed\s*[:\-]\s*(.+)$/i, build: (m) => `تمت المراجعة: ${arMonths(m[1])}` },
  // "Available across X currencies"
  { re: /^Available\s+across\s+(\d[\d,]*)\s+currenc(?:y|ies)\b\.?$/i,
    build: (m) => `متوفر بـ ${m[1]} عملات` },
  // "15 days" / "30 day"
  { re: /^(\d[\d,]*)\s+days?\b\.?$/i, build: (m) => `${m[1]} أيام` },
  // "5 products"
  { re: /^(\d[\d,]*)\s+products?\b\.?$/i, build: (m) => `من ${m[1]} منتج` },
  // "3 cities"
  { re: /^(\d[\d,]*)\s+cities\b\.?$/i, build: (m) => `${m[1]} مدن` },
  // "of 12 orders" / "of 5 products"
  { re: /^of\s+(\d[\d,]*)\s+orders?\b\.?$/i, build: (m) => `من أصل ${m[1]} طلبيات` },
  { re: /^of\s+(\d[\d,]*)\s+products?\b\.?$/i, build: (m) => `من أصل ${m[1]} منتجات` },
  // "out of N product(s)" / "out of N order(s)"
  { re: /^out\s+of\s+(\d[\d,]*)\s+products?\b\.?$/i, build: (m) => `من أصل ${m[1]} منتج` },
  { re: /^out\s+of\s+(\d[\d,]*)\s+orders?\b\.?$/i, build: (m) => `من أصل ${m[1]} طلبية` },
  // "Delivered" variants like "Deliv." or "deliv"
  { re: /^Deliv\.?$/i, build: () => `التوصيل` },

  // "0 new", "5 new" — notification banner
  { re: /^(\d[\d,]*)\s+new$/i, build: (m) => `${m[1]} جديد` },
  // "£0.00 pending" / "£12.50 pending"
  { re: /^([£$€])\s*([\d.,]+)\s+pending$/i, build: (m) => `${m[1]}${m[2]} معلّق` },
  // "0% failure rate" / "−70% failure rate"
  { re: /^[−\-+]?\s*([\d.]+)%\s+failure\s+rate$/i, build: (m) => `نسبة الطلبات غير المستلمة ${m[1]}%` },
  // "per unit" / "per unit × 1"
  { re: /^per\s+unit$/i, build: () => `للوحدة` },
  { re: /^([£$€]?[\d.,]*)\s*per\s+unit\s*[×x]\s*(\d+)$/i,
    build: (m) => `${m[1] ? m[1] + ' ' : ''}للوحدة × ${m[2]}` },
  // "Draft · not sent" / "Draft not sent"
  { re: /^Draft\s*[·.\-]?\s*not\s+sent$/i, build: () => `مسودة · لم تُرسل` },

  // "Paid June 2025" / "Pending July 2025" (admin period labels)
  { re: /^Paid\s+([A-Z][a-z]+\s+\d{4})$/, build: (m) => `تم الدفع ${arMonths(m[1])}` },
  { re: /^Pending\s+([A-Z][a-z]+\s+\d{4})$/, build: (m) => `معلّق ${arMonths(m[1])}` },
  // "Failed to load: <message>"
  { re: /^Failed\s+to\s+load\s*[:\-]\s*(.+)$/i, build: (m) => `فشل التحميل: ${m[1]}` },
  // "Size: M" / "Colour: Red"
  { re: /^Size\s*:\s*(.+)$/i, build: (m) => `المقاس: ${m[1]}` },
  { re: /^Colou?r\s*:\s*(.+)$/i, build: (m) => `اللون: ${m[1]}` },
  // "Note: ..."
  { re: /^Note\s*:\s*(.+)$/i, build: (m) => `ملاحظة: ${m[1]}` },

  // "Marketer confirmed payment · 16 May 2026"
  { re: /^Marketer\s+confirmed\s+payment\s*[·\-]\s*(.+)$/i,
    build: (m) => `المسوّق أكد الدفع · ${arMonths(m[1])}` },
  // "Upfront fee paid · 11.25د.ل · 16 May 2026" (any order/extras)
  { re: /^Upfront\s+fee\s+paid\s*[·\-]\s*(.+)$/i,
    build: (m) => `تم دفع العربون · ${arMonths(m[1])}` },
  // "Delivered ✓" / "Delivered ✔"
  { re: /^Delivered\s*[✓✔]+$/i, build: () => `تم التسليم ✓` },
  // "Approved · forwarded to business ✓ ✓"
  { re: /^Approved\s*[·\-]\s*forwarded\s+to\s+business[\s✓✔]*$/i,
    build: () => `تمت الموافقة · تم التحويل للتاجر ✓ ✓` },
  // "wallet د.ل" / "wallet د.إ" / "wallet $"
  { re: /^wallet\s+(.+)$/i, build: (m) => `المحفظة ${m[1]}` },
  { re: /^(.+)\s+wallet$/i, build: (m) => `المحفظة ${m[1]}` },
  // "1 cities available" / "12 cities available"
  { re: /^(\d[\d,]*)\s+cities\s+available\b\.?$/i,
    build: (m) => {
      const n = parseInt(m[1].replace(/,/g, ""), 10);
      if (n === 1) return "مدينة واحدة متاحة لهذا المنتج";
      if (n === 2) return "مدينتين متاحة لهذا المنتج";
      return `${n} مدن متاحة لهذا المنتج`;
    } },
  // "1 countries available for this product"
  { re: /^(\d[\d,]*)\s+countries\s+available(?:\s+for\s+this\s+product)?\b\.?$/i,
    build: (m) => {
      const n = parseInt(m[1].replace(/,/g, ""), 10);
      if (n === 1) return "دولة واحدة متاحة لهذا المنتج";
      if (n === 2) return "دولتين متاحة لهذا المنتج";
      return `${n} دول متاحة لهذا المنتج`;
    } },
  // "For Benghazi, LY · set by business owner"
  { re: /^For\s+(.+?)\s*[·\-]\s*set\s+by\s+business\s+owner\b\.?$/i,
    build: (m) => `لـ ${m[1]} · يحدده صاحب تاجر` },
  { re: /^set\s+by\s+business\s+owner\b\.?$/i,
    build: () => `يحدده صاحب تاجر` },
  // "commission %28.6" / "commission 28.6%"
  { re: /^commission\s+%?([\d.]+)%?$/i, build: (m) => `العمولة ${m[1]}%` },
  // "%28.6 commission" / "28.6% commission"
  { re: /^%?([\d.]+)%?\s+commission$/i, build: (m) => `عمولة ${m[1]}%` },
  // Product card meta: "£25.00 · 28.6% commission · 3 cities"
  { re: /^(.+?)\s*·\s*(.+?)\s+commission\s*·\s*(\d[\d,]*)\s+cities$/i,
    build: (m) => `${m[1]} · عمولة ${m[2]} · ${m[3]} مدن` },
  // "28.6% commission · 3 cities"
  { re: /^(.+?)\s+commission\s*·\s*(\d[\d,]*)\s+cities$/i,
    build: (m) => `عمولة ${m[1]} · ${m[2]} مدن` },
  // "£25.00 · 28.6% commission" (saved/favorites card meta)
  { re: /^(.+?)\s*·\s*(.+?)\s+commission$/i,
    build: (m) => `${m[1]} · عمولة ${m[2]}` },
  // "Awaiting marketer confirmation · 16 May 2026"
  { re: /^Awaiting\s+marketer\s+confirmation\s*[·\-]\s*(.+)$/i,
    build: (m) => `بانتظار تأكيد المسوّق · ${arMonths(m[1])}` },
  // "Your commission (×2)" / "Your commission ×3"
  { re: /^Your\s+commission\s*[\(]?\s*[×x]\s*(\d+)\s*[\)]?$/i,
    build: (m) => `مكسبك من كل بيعة (×${m[1]})` },
  // "Full payment received · £25.00 · 16 May 2026"
  { re: /^Full\s+payment\s+received(?:\s*[·\-]\s*(.+))?$/i,
    build: (m) => m[1] ? `تم استلام كامل المبلغ · ${arMonths(m[1])}` : `تم استلام كامل المبلغ` },
  // "<n> in stock" (public product share page availability line)
  { re: /^(\d[\d,]*)\s+in\s+stock$/i, build: (m) => {
      const n = parseInt(m[1].replace(/,/g, ""), 10);
      const piece = n === 1 ? "قطعه واحده" : n === 2 ? "قطعتين" : (n > 10 ? `${n} قطعه` : `${n} قطع`);
      return `متوفر ${piece}`;
    } },
  // "<n> left" (public product share page per-variant stock)
  { re: /^(\d[\d,]*)\s+left$/i, build: (m) => {
      const n = parseInt(m[1].replace(/,/g, ""), 10);
      const piece = n === 1 ? "قطعه واحده" : n === 2 ? "قطعتين" : (n > 10 ? `${n} قطعه` : `${n} قطع`);
      return `متبقي ${piece}`;
    } },
];

// Normalize then lookup. Returns the translation, or null if not in dict.
export function translate(text: string): string | null {
  const trimmed = text.trim();
  if (!trimmed) return null;

  // 1) Exact match (case-preserving)
  if (DICT[trimmed]) return DICT[trimmed];

  // 2) Strip trailing punctuation for a second-chance exact match
  const stripped = trimmed.replace(/[.,;:!?]+$/, "");
  if (stripped !== trimmed && DICT[stripped]) return DICT[stripped];

  // 3) Case-insensitive match
  const lower = stripped.toLowerCase();
  if (DICT_CI[lower]) return DICT_CI[lower];

  // 4) Pattern match (preserves dynamic numbers / IDs / names)
  for (const p of PATTERNS) {
    const m = stripped.match(p.re);
    if (m) return p.build(m);
  }

  return null;
}
