...
async function toggleStatus(id){const p=products.find(x=>x.id===id);if(!p)return;const next=p.status==='active'?'paused':'active';if(next==='paused'){const __n=mpActiveMarketerCount(p);if(__n>0){try{await window.LateenAPI.requestPauseProduct(id);}catch(e){console.error(e);}__showPauseBlockedCard();return;}}try{await window.LateenAPI.setStatus(id,next);await loadProducts();}catch(e){console.error(e);alert(__ar()?'فشل تحديث الحالة':'Failed to update status');}}
