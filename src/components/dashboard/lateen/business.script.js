...
function computeOrdFin(o){const subtotal=o.total;const commissionPct=subtotal>0?Math.round((o.commission/subtotal)*100):0;const platformPct=(o.price>0&&o.qty>0)?Math.round(((o.platformFee/o.qty)/o.price)*100):0;const base=o.price*o.qty;const net=base-o.commission-o.platformFee;const hasDelivery=Number(o.delivery)>0;const hasShipping=Number(o.shipping)>0;let extra=null;if(hasDelivery&&hasShipping){extra={label:'Total delivery and shipping',value:base+Number(o.delivery)+Number(o.shipping)-o.commission-o.platformFee};}else if(hasDelivery){extra={label:'Total with delivery fee',value:base+Number(o.delivery)-o.commission-o.platformFee};}else if(hasShipping){extra={label:'Total with shipping fee',value:base+Number(o.shipping)-o.commission-o.platformFee};}return{subtotal,commissionPct,platformPct,net,extra};}
