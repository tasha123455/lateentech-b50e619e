...
let products=[];
let requireAdditionalPhone=false;
...
function __applyRequireAdditionalPhoneUI(){const btn=document.getElementById('require-wa-toggle');if(!btn)return;btn.classList.toggle('on',!!requireAdditionalPhone);btn.setAttribute('aria-checked',requireAdditionalPhone?'true':'false');}
async function onToggleRequireAdditionalPhone(){if(__frozenBlock())return;const btn=document.getElementById('require-wa-toggle');const next=!requireAdditionalPhone;if(btn){btn.disabled=true;btn.style.opacity='.7';}try{await window.LateenAPI.setRequireAdditionalPhone(next);requireAdditionalPhone=next;if(window.__profileData)window.__profileData.require_additional_phone=next;__applyRequireAdditionalPhoneUI();await loadProducts();}catch(e){console.error('[Lateen] setRequireAdditionalPhone',e);alert(__ar()?'تعذر حفظ الإعداد':'Could not save this setting.');}finally{if(btn){btn.disabled=false;btn.style.opacity='';}}}
