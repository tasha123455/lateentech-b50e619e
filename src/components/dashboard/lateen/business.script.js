...
function mpSlidesHtml(p){
  ...
  return photos.map(url=>`<div class="mp-p-thumb-slide" style="width: ${100/len}%; flex: 0 0 ${100/len}%;"><img src="${mpEsc(url)}" alt="${mpEsc(p.name)}" loading="lazy" decoding="async"/></div>`).join('');
}
    ...
    <div class="mp-p-head" onclick="mpToggleCard('${p.id}')">
      <div class="mp-p-thumb-wrap" id="mp-thumb-wrap-${p.id}" onclick="event.stopPropagation();mpOpenLightbox('${p.id}')">
        <div class="mp-p-thumb" id="mp-thumb-${p.id}">
          ...
          ${p.category?`<span class="mp-thumb-category" data-no-i18n>${mpEsc(__ar()?(CATEGORY_ITEM_AR[p.category]||p.category):p.category)}</span>`:''}
          ${mpDotsHtml(p)}
        </div>
      </div>
      <div class="mp-p-info">
      ...
      <div class="order-card ${isExp?'expanded':''}" data-status="${o.status}" data-id="${o.id}">
        ...
        <div class="detail-body">
          ...
          <div class="fin-box">
            ...
            <div class="fin-row"><span class="k">Quantity (${o.qty})</span><span class="v">${om(o.price*o.qty)}</span></div>
