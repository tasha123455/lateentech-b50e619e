...
function bdRenderBreakdown(){const data=__bdGenerateData(__bdSelected);const mEl=document.getElementById('bd-marketers-val');if(mEl)mEl.textContent=data.marketers;const grid=document.getElementById('bd-grid');if(!grid)return;const total=data.succeeded+data.failed;const succPct=total>0?Math.round((data.succeeded/total)*100):0;const failPct=total>0?Math.round((data.failed/total)*100):0;const sym=(typeof window.__bizSelSym==='function')?window.__bizSelSym():'$';const ar=__ar();const succLbl=ar?'الطلبات تم تسليمها':'Succeeded';const failLbl=ar?'الطلبات لم يتم تسليمها':'Failed';const succSub=ar?('من أصل '+__ordFrac(total)):(succPct+'%');const failSub=ar?('من أصل '+__ordFrac(total)):(failPct+'%');const piecesLbl=ar?'قطع تم بيعها':'Pieces sold';const netLbl=ar?'صافي الأرباح منذ إنشاء الحساب':'Net earnings';grid.innerHTML=`
  ...
  `;}
const __bdDaysAr={'Sun':'الأحد','Mon':'الإثنين','Tue':'الثلاثاء','Wed':'الأربعاء','Thu':'الخميس','Fri':'الجمعة','Sat':'السبت'};
const __bdMonthsAr={'Jan':'يناير','Feb':'فبراير','Mar':'مارس','Apr':'أبريل','May':'مايو','Jun':'يونيو','Jul':'يوليو','Aug':'أغسطس','Sep':'سبتمبر','Oct':'أكتوبر','Nov':'نوفمبر','Dec':'ديسمبر'};
const __bdRangeMeta={
  daily:{key:'day',defaultLabel:'Day',defaultLabelAr:'اليوم',list:null},
  monthly:{key:'month',defaultLabel:'Month',defaultLabelAr:'الشهر',list:null},
  yearly:{key:'year',defaultLabel:'Year',defaultLabelAr:'السنة',list:null}
};
function __bdUpdateTabLabel(filterKey){
  const rangeName=Object.keys(__bdRangeMeta).find(r=>__bdRangeMeta[r].key===filterKey);
  const tab=document.querySelector(`.bd-range-tab[data-range="${rangeName}"]`);
  if(!tab)return;
  const val=__bdSelected[filterKey];
  const ar=__ar();
  const defLabel=ar?__bdRangeMeta[rangeName].defaultLabelAr:__bdRangeMeta[rangeName].defaultLabel;
  let displayVal=val;
  if(val&&ar){
    if(filterKey==='day') displayVal=__bdDaysAr[val]||val;
    if(filterKey==='month') displayVal=__bdMonthsAr[val]||val;
  }
  tab.innerHTML=(displayVal?displayVal:defLabel)+' <span class="bd-chev">▾</span>';
  tab.classList.toggle('active',!!val);
  __bdUpdateAllTabState();
}
...
function __bdBuildDropdown(listEl,keys,filterKey,rangeName){
  if(!listEl)return;
  listEl.innerHTML='';
  const ar=__ar();
  const clearItem=document.createElement('div');
  clearItem.className='bd-dd-item clear';
  clearItem.textContent=ar?'إلغاء التحديد':'Clear selection';
  clearItem.addEventListener('click',e=>{
    e.stopPropagation();
    __bdSelected[filterKey]=null;
    __bdUpdateTabLabel(filterKey);
    __bdCloseDropdown(rangeName);
    bdRenderBreakdown();
  });
  ...
  keys.forEach(key=>{
    ...
    let displayKey=key;
    if(ar){
      if(filterKey==='day') displayKey=__bdDaysAr[key]||key;
      if(filterKey==='month') displayKey=__bdMonthsAr[key]||key;
    }
    item.textContent=displayKey;
    item.addEventListener('click',e=>{
      e.stopPropagation();
      __bdSelected[filterKey]=key;
      __bdUpdateTabLabel(filterKey);
      __bdCloseDropdown(rangeName);
      bdRenderBreakdown();
    });
...
function bdTabClick(range,tabEl){
  if(range==='all'){
    __bdCloseAllDropdowns();
    __bdSelected.day=null;
    __bdSelected.month=null;
    __bdSelected.year=null;
    const ar=__ar();
    Object.keys(__bdRangeMeta).forEach(r=>{
      const t=document.querySelector(`.bd-range-tab[data-range="${r}"]`);
      if(t){
        const defLabel=ar?__bdRangeMeta[r].defaultLabelAr:__bdRangeMeta[r].defaultLabel;
        t.innerHTML=defLabel+' <span class="bd-chev">▾</span>';
  ...
  const meta=__bdRangeMeta[range];
  if(!meta||!meta.list)return;
  const isOpen=meta.list.classList.contains('open');
  Object.keys(__bdRangeMeta).forEach(r=>{if(r!==range)__bdCloseDropdown(r);});
  if(isOpen){
    __bdCloseDropdown(range);
  }else{
    meta.list.classList.add('open');
    tabEl.classList.add('open');
  }
}
...
function __countryNameLbl(code){
  if(typeof __ar==='function'&&__ar()){
    const arCountries={
      'LY': 'ليبيا',
      'AE': 'الإمارات',
      'SA': 'السعودية',
    ...
    if(arCountries[code]) return arCountries[code];
  }
...
function mpStatusBadges(p){
  ...
  if(p.qty===0)b.push(`<span class="mp-status-pill mp-status-outofstock"><span class="mp-dot-ind"></span>${__ar()?'نفدت الكميه':'Out of stock'}</span>`);
...
function mpSlidesHtml(p){
  ...
  if(!photos.length){
    ...
    return `<div class="mp-p-thumb-slide mp-p-thumb-empty" style="width: 100%; flex: 0 0 100%;">${icon}</div>`;
  }
  return photos.map(url=>`<div class="mp-p-thumb-slide" style="width: ${100/len}%; flex: 0 0 ${100/len}%;"><img src="${mpEsc(url)}" alt="${mpEsc(p.name)}" loading="eager"/></div>`).join('');
}
...
function mpShippingFold(p){
  ...
  const boxes=codes.map(code=>{
    ...
    const rows=Object.entries(cities).map(([city,c])=>`<div class="mp-city-row"><span><b>${mpEsc(code==='LY'?__cityLbl(city):city)}</b></span><span class="mp-dfee">${__ar()?'التوصيل':'Delivery'} <b>${fmtP((c&&c.delivery)||0)}</b></span></div>`).join('');
    return `<div class="mp-country-box">
      <div class="mp-country-head">
        <span class="mp-cname">${COUNTRY_FLAGS[code]||'🌐'} ${mpEsc(__countryNameLbl(code))}</span>
    ...
    <div class="mp-p-head" onclick="mpToggleCard('${p.id}')">
      <div class="mp-p-thumb-wrap" id="mp-thumb-wrap-${p.id}" onclick="event.stopPropagation();mpOpenLightbox('${p.id}')">
        <div class="mp-p-thumb" id="mp-thumb-${p.id}">
          <div class="mp-p-thumb-track" id="mp-thumb-track-${p.id}" style="width: ${((p.photos||[]).filter(Boolean).length||1)*100}%; transform: translateX(-${(mpPhotoIndex[p.id]||0)*(100/((p.photos||[]).filter(Boolean).length||1))}%)">${mpSlidesHtml(p)}</div>
          ${p.category?`<span class="mp-thumb-category" data-no-i18n>${mpEsc(__ar()?(CATEGORY_ITEM_AR[p.category]||p.category):(CATEGORY_ITEM_EN[p.category]||p.category))}</span>`:''}
          ${mpDotsHtml(p)}
        </div>
      </div>
      <div class="mp-p-info">
...
function mpInitSwipe(pid){
  ...
  wrap.addEventListener('touchend',e=>{
    ...
    if(Math.abs(dx)>Math.abs(dy)&&Math.abs(dx)>40){
      ...
      const next=dx<0?Math.min(cur+1,cnt-1):Math.max(cur-1,0);
...
function recomputeAnalytics(){
  ...
  set('stat-gross',__money(totGross,'$','USD'));set('stat-pieces',totPieces.toLocaleString());set('stat-pieces-sub','out of '+products.length+' product'+(products.length===1?'':'s'));set('stat-ok',totOk.toLocaleString());set('stat-ok-sub',totDone>0?Math.round(totOk/totDone*100)+'%':'0%');set('stat-fail',totFail.toLocaleString());set('stat-fail-sub',totDone>0?Math.round(totFail/totDone*100)+'%':'0%');set('stat-marketers',marketerSet.size.toLocaleString());set('brk-gross',__money(totGross,'$','USD'));set('brk-comm','−'+__money(totComm,'$','USD'));set('brk-plat','−'+__money(totPlat,'$','USD'));set('brk-net',__money(totGross-totComm-totPlat,'$','USD'));
...
function buildOrdStepper(s){if(s==='failed')return`<div class="progress-failed">${__ar()?'طلب فاشل':'Order failed'}</div>`;const si=ST[s]?.step??0;let h='<div class="progress">';STEPS.forEach((step,i)=>{h+=`<div class="step${i===si?' active':''}"><div class="bubble"></div><div class="lbl">${step}</div></div>`;if(i<STEPS.length-1)h+=`<div class="line"></div>`;});h+='</div>';return h;}
function buildOrdActions(o){if(o.status==='delivered'||o.status==='failed'){return`<div class="actions"><button class="btn btn-lock" disabled>${o.status==='delivered'?'✓ Delivered':(__ar()?'فشل':'Failed')}</button></div>`;}let b='';if(o.status==='new')b+=`<button class="btn btn-confirm" onclick="event.stopPropagation();advance('${o.id}','confirmed')">Confirm order</button>`;if(o.status==='confirmed')b+=`<button class="btn btn-deliver" onclick="event.stopPropagation();advance('${o.id}','delivered')">Mark delivered</button>`;if(o.status==='new'||o.status==='confirmed')b+=`<button class="btn btn-fail" onclick="event.stopPropagation();advance('${o.id}','failed')">${__ar()?'فشل':'Failed'}</button>`;return`<div class="actions">${b}</div>`;}
      ...
      <div class="order-card ${isExp?'expanded':''}" data-status="${o.status}" data-id="${o.id}">
        ...
        <div class="detail-body">
          ...
          <div class="box">
            ...
            <div class="r"><span class="k">${__ar() ? 'الكميه' : 'Quantity'}</span><span class="v">${o.qty}</span></div>
            ${(o.size||o.color)?`<div class="r variant-combo"><span class="k">${variantLabel}</span><div class="v variant-combo-val"><div class="variant-combo-text">${o.size||o.color}</div>${variantThumb}</div></div>`:''}
          </div>
...
function __applyAvatarTo(el,url,initials){if(!el)return;el.style.backgroundImage='';if(url){el.textContent='';const img=document.createElement('img');img.decoding='async';img.loading='eager';img.alt='';img.style.cssText='width:100%;height:100%;object-fit:contain;object-position:center;display:block;border-radius:inherit;';img.onerror=function(){el.textContent=initials||'··';};img.src=url;el.appendChild(img);}else{el.textContent=initials||'··';}}
...
window.__bizSelSym=function(){const e=window.__bizEarnByCur||{};const k=window.__bizWalletCur||Object.keys(e)[0]||'USD';const symbolMap={LYD:'د.ل',USD:'$',GBP:'£',EUR:'€'};return (e[k]&&e[k].sym)||symbolMap[k]||'$';};
...
recomputeAnalytics=function(){
  ...
  try{
    ...
    const codes=Object.keys(byCur);if(!window.__bizWalletCur)window.__bizWalletCur=codes[0]||'USD';
    const sel=window.__bizWalletCur;const symbolMap={LYD:'د.ل',USD:'$',GBP:'£',EUR:'€'};const sd=byCur[sel]||{sym:symbolMap[sel]||'$',gross:0,comm:0,plat:0,net:0};
  ...
  root.innerHTML=list.map(n=>{
    ...
    if(expandable&&n.data){
      ...
      if(d){
        ...
        const photo=d.product_photo&&/^(https?:|data:|\/)/.test(String(d.product_photo))?`<div style="margin:-2px 0 10px 0"><img src="${esc(d.product_photo)}" alt="" style="width:100%;max-height:170px;object-fit:contain;border-radius:10px;display:block"/></div>`:'';
