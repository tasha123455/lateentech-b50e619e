...
function __stepSimpleQty(d){const el=document.getElementById('f-simple-qty');if(!el)return;const cur=Math.max(1,parseInt(el.value||'1',10)||1);const nv=Math.max(1,cur+d);el.value=String(nv);simpleQty=String(nv);}
function __onSimpleQtyInput(el){let v=parseInt(el.value||'',10);if(!Number.isFinite(v)||v<1)v=1;el.value=String(v);simpleQty=String(v);}
