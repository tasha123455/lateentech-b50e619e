...
async function toggleStatus(id){const p=products.find(x=>x.id===id);if(!p)return;const next=p.status==='active'?'paused':'active';if(next==='paused'){const __n=mpActiveMarketerCount(p);if(__n>0){try{await window.LateenAPI.requestPauseProduct(id);}catch(e){console.error(e);}alert(__ar()?'تعذر إيقاف المنتج مؤقتا\n\nعلى المنتج هذا فيه طلبات نشطة من المسوّقين. أول ما تتصفى كل الطلبات، بيتوقف المنتج تلقائيا.':'Unable to Pause Product\n\nThis product has active marketer orders. It will be paused automatically once all orders have been cleared.');return;}}try{await window.LateenAPI.setStatus(id,next);await loadProducts();}catch(e){console.error(e);alert('Failed to update status');}}
