...
function updateProductSummaryCard(){const price=parseFloat(document.getElementById('f-price').value)||0;const pct=parseFloat(document.getElementById('f-comm-pct').value)||0;const fixed=parseFloat(document.getElementById('f-comm-fixed').value)||0;const code=selectedCurrency?selectedCurrency.code:'LYD';const codeLbl=(__ar()&&code==='LYD')?'د.ل':code;const{comm,platform,commPct}=calcFees(price,pct,fixed,commMode);const total=price-comm-platform;const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};set('sum-price',`${price.toFixed(2)} ${codeLbl}`);set('sum-marketer-label',__ar()?`عمولة المسوق (${commPct}%)`:`Marketer commission (${commPct}%)`);set('sum-marketer-amt',`${comm.toFixed(2)} ${codeLbl}`);set('sum-platform-amt',`${platform.toFixed(2)} ${codeLbl}`);set('sum-total',`${total.toFixed(2)} ${codeLbl}`);}
...
function mpSlidesHtml(p){
  ...
  return photos.map(url=>`<div class="mp-p-thumb-slide"><img src="${mpEsc(url)}" alt="${mpEsc(p.name)}" loading="eager" decoding="async"/></div>`).join('');
}
