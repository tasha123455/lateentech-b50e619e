...
function __bdActiveMarketersNow(){
  try{
    if(typeof orders==='undefined'||!Array.isArray(orders))return 0;
    const ACTIVE_ST=new Set(['pending','approved','confirmed']);
    const liveProductIds=new Set((typeof products!=='undefined'&&Array.isArray(products)?products:[]).map(p=>p.id));
    const ids=new Set(orders.filter(o=>o.marketerId&&ACTIVE_ST.has(o._status)&&liveProductIds.has(o.productId)).map(o=>o.marketerId));
    return ids.size;
  }catch(e){return 0;}
}
