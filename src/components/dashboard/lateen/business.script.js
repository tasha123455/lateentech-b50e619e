...
function __money(n,sym,code){const a=parseFloat(n||0).toFixed(2);const cc=(code||__SYM2CODE[__stripDirMarks(sym||'')]||'').toString().toUpperCase();const r=__rawSym(sym||'£',cc);const isAr=(typeof document!=='undefined'&&document.documentElement.lang==='ar');return isAr?(r+a):(cc?(a+' '+cc):(r+a));}
function __moneyH(n,sym,code){const a=parseFloat(n||0).toFixed(2);const cc=(code||__SYM2CODE[__stripDirMarks(sym||'')]||'').toString().toUpperCase();const r=__rawSym(sym||'£',cc);const isAr=(typeof document!=='undefined'&&document.documentElement.lang==='ar');const symHtml='<span class="cur-sym">'+__escH(r)+'</span>';const codeHtml=cc?'<span class="cur-sym">'+__escH(cc)+'</span>':'';return isAr?(symHtml+__escH(a)):(cc?(__escH(a)+' '+codeHtml):(symHtml+__escH(a)));}
...
function bdRenderBreakdown(){const data=__bdGenerateData(__bdSelected);const mEl=document.getElementById('bd-marketers-val');if(mEl)mEl.textContent=data.marketers;const grid=document.getElementById('bd-grid');if(!grid)return;const total=data.succeeded+data.failed;const succPct=total>0?Math.round((data.succeeded/total)*100):0;const failPct=total>0?Math.round((data.failed/total)*100):0;const sym=(typeof window.__bizSelSym==='function')?window.__bizSelSym():'£';grid.innerHTML=`
    <div class="bd-box"><div class="bd-box-label">Net earnings</div><div class="bd-box-value">${__money(data.earnings,sym,window.__bizWalletCur)}</div></div>
    <div class="bd-box"><div class="bd-box-label">Pieces sold</div><div class="bd-box-value">${data.pieces}</div></div>
    <div class="bd-box"><div class="bd-box-label">Succeeded</div><div class="bd-box-value green">${data.succeeded} <span style="font-size:12px;color:var(--color-text-tertiary);font-weight:500;">(${succPct}%)</span></div></div>
    <div class="bd-box"><div class="bd-box-label">Failed</div><div class="bd-box-value red">${data.failed} <span style="font-size:12px;color:var(--color-text-tertiary);font-weight:500;">(${failPct}%)</span></div></div>
  `;}
...
function renderCategoryList(filter){
  ...
  CATEGORY_DATA.forEach(section=>{
    ...
    const items=section.items.filter(it=>groupMatches||it.toLowerCase().includes(q));
    if(!items.length)return;
    any=true;
    html+=`<div class="lp-category-group-label">${section.group}</div>`;
    items.forEach(it=>{html+=`<div class="lp-category-option" onclick="selectCategory('${it.replace(/'/g,"\\'")}')">${it}</div>`;});
  });
  if(!any)html='<div class="lp-category-empty">No matching categories</div>';
...
function selectCategory(name){selectedCategory=name;const v=document.getElementById('category-picker-value');v.textContent=name;v.classList.remove('placeholder');closeCategoryPanel();}
...
function updateConversionNote(){const note=document.getElementById('conversion-note');if(!note)return;const price=parseFloat(document.getElementById('f-price').value)||0;const code=selectedCurrency?selectedCurrency.code:'LYD';if(commMode==='pct'){const pct=parseFloat(document.getElementById('f-comm-pct').value)||0;const amt=pctOf(price,pct);note.innerHTML=`Fixed equivalent at current price: <b>${amt.toFixed(2)} ${code}</b> per sale`;}else{const fixed=parseFloat(document.getElementById('f-comm-fixed').value)||0;const pct=price?((fixed/price)*100):0;note.innerHTML=`Percentage equivalent at current price: <b>${pct.toFixed(1)}%</b> per sale`;}}
function updateProductSummaryCard(){const price=parseFloat(document.getElementById('f-price').value)||0;const pct=parseFloat(document.getElementById('f-comm-pct').value)||0;const fixed=parseFloat(document.getElementById('f-comm-fixed').value)||0;const code=selectedCurrency?selectedCurrency.code:'LYD';const{comm,platform,commPct}=calcFees(price,pct,fixed,commMode);const total=price-comm-platform;const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};set('sum-price',`${price.toFixed(2)} ${code}`);set('sum-marketer-label',`Marketer commission (${commPct}%)`);set('sum-marketer-amt',`${comm.toFixed(2)} ${code}`);set('sum-platform-amt',`${platform.toFixed(2)} ${code}`);set('sum-total',`${total.toFixed(2)} ${code}`);}
...
function addVariantGroup(){variantGroups.push({id:groupIdCounter++,name:'',placeholder:'Variant name',valPlaceholder:__ar()?'مثال: أسود أو XL':'Black or XL',items:[]});renderVariantGroups();setTimeout(()=>{const ins=document.querySelectorAll('.vg-name-input');if(ins.length)ins[ins.length-1].focus();},50);}
...
function addVariant(gid){const g=variantGroups.find(x=>x.id===gid);if(!g)return;g.items.push({id:variantIdCounter++,val:'',qty:'',photo:''});renderVariantGroups();setTimeout(()=>{const ins=document.querySelectorAll(`#vg-list-${gid} input.vb-val-input`);if(ins.length)ins[ins.length-1].focus();},50);}
...
function updateVariantQty(gid,id,val){const g=variantGroups.find(x=>x.id===gid);if(!g)return;const it=g.items.find(x=>x.id===id);if(it)it.qty=val;}
...
function openAddForm(p){photos=p?[...(p.photos||[])]:[];variantGroups=hydrateVariantGroups(p);zones=p?JSON.parse(JSON.stringify(p.delivery)):{};Object.keys(zones).forEach(code=>{const z=zones[code];if(!z.cities)z.cities={};let s=0;Object.values(z.cities).forEach(c=>{const cs=Number(c.shipping)||0;if(cs>s)s=cs;});if(z.shipping==null||z.shipping==='')z.shipping=s||'';});commMode=p?.commMode||'pct';selectedCurrency=p?.currency||null;currentCode=ensureUniqueCode(p?.code);document.getElementById('product-code-display').textContent=currentCode;document.getElementById('form-title').textContent=p?'Edit product':'Add a product';document.getElementById('f-name').value=p?.name||'';document.getElementById('f-desc').value=p?.desc||'';document.getElementById('f-price').value=p?.price||'';document.getElementById('f-cost').value=p?.costPrice||'';selectedCategory=p?.category||'';const catVal=document.getElementById('category-picker-value');if(selectedCategory){catVal.textContent=selectedCategory;catVal.classList.remove('placeholder');}else{catVal.textContent='Select category';catVal.classList.add('placeholder');}document.getElementById('f-comm-pct').value=p?.commPct||'';document.getElementById('f-comm-fixed').value=p?.commFixed||'';if(selectedCurrency){document.getElementById('curr-flag').textContent=selectedCurrency.flag;document.getElementById('curr-name').textContent=selectedCurrency.name+' ('+selectedCurrency.code+')';document.getElementById('curr-symbol-label').textContent=selectedCurrency.symbol;document.getElementById('price-prefix').textContent=selectedCurrency.symbol;}else{document.getElementById('curr-flag').textContent='🌐';document.getElementById('curr-name').textContent='Select a currency';document.getElementById('curr-symbol-label').textContent='';document.getElementById('price-prefix').textContent='—';}updateCommFixedUnitLabel();setCommMode(commMode);renderPhotoGrid();renderVariantGroups();renderZoneBuilderList();updateConversionNote();updateProductSummaryCard();document.getElementById('form-overlay').classList.add('open');}
function closeForm(){document.getElementById('form-overlay').classList.remove('open');editingId=null;photos=[];variantGroups=[];zones={};selectedCurrency=null;selectedCategory='';currDropdownOpen=false;countryDropdownOpen=false;closeCategoryPanel();}
...
function mpFmt(p){const sym=p.currency?__wrapArSym(p.currency.symbol,p.currency.code):'';return (n)=>__money(n,sym,p.currency&&p.currency.code);}
...
function mpSlidesHtml(p){
  ...
  return photos.map(url=>`<div class="mp-p-thumb-slide"><img src="${mpEsc(url)}" alt="${mpEsc(p.name)}" loading="lazy" decoding="async"/></div>`).join('');
}
...
function mpAnalyticsSection(p){
  const period=mpPeriod[p.id]||'all';
  return `<div class="mp-analytics-panel-inner">
    <div class="mp-analytics-period-row" id="mp-period-row-${p.id}">
      <div class="mp-period-chip${period==='day'?' sel':''}" data-pid="${p.id}" data-period="day" onclick="event.stopPropagation();mpSetPeriod('${p.id}','day')">${__ar()?'اليوم':'Today'}</div>
      <div class="mp-period-chip${period==='month'?' sel':''}" data-pid="${p.id}" data-period="month" onclick="event.stopPropagation();mpSetPeriod('${p.id}','month')">${__ar()?'هذا الشهر':'This month'}</div>
      <div class="mp-period-chip${period==='year'?' sel':''}" data-pid="${p.id}" data-period="year" onclick="event.stopPropagation();mpSetPeriod('${p.id}','year')">${__ar()?'هذا العام':'This year'}</div>
      <div class="mp-period-chip${period==='all'?' sel':''}" data-pid="${p.id}" data-period="all" onclick="event.stopPropagation();mpSetPeriod('${p.id}','all')">${__ar()?'كل الوقت':'All time'}</div>
    </div>
...
function mpShippingFold(p){
  ...
  const boxes=codes.map(code=>{
    ...
    return `<div class="mp-country-box">
      <div class="mp-country-head">
        <span class="mp-cname">${COUNTRY_FLAGS[code]||'🌐'} ${mpEsc(COUNTRY_NAMES[code]||code)}</span>
  ...
  return `<div class="mp-fold-section">
    <div class="mp-fold-head" onclick="event.stopPropagation();mpToggleFold('shipping-${p.id}')">
      <div><div class="mp-fold-label">${__ar()?'الشحن والتوصيل':'Shipping & delivery'}</div><div class="mp-fold-sub">${codes.length} ${codes.length===1?(__ar()?'دولة':'country'):(__ar()?'دول':'countries')}</div></div>
    ...
    <div class="mp-p-head" onclick="mpToggleCard('${p.id}')">
      <div class="mp-p-thumb-wrap" id="mp-thumb-wrap-${p.id}" onclick="event.stopPropagation();mpOpenLightbox('${p.id}')">
        <div class="mp-p-thumb" id="mp-thumb-${p.id}">
          ...
          ${p.category?`<span class="mp-thumb-category">${mpEsc(p.category)}</span>`:''}
          ${mpDotsHtml(p)}
        </div>
      </div>
      <div class="mp-p-info">
...
function recomputeAnalytics(){
  ...
  set('stat-gross',__money(totGross,'£','GBP'));set('stat-pieces',totPieces.toLocaleString());set('stat-pieces-sub','out of '+products.length+' product'+(products.length===1?'':'s'));set('stat-ok',totOk.toLocaleString());set('stat-ok-sub',totDone>0?Math.round(totOk/totDone*100)+'%':'0%');set('stat-fail',totFail.toLocaleString());set('stat-fail-sub',totDone>0?Math.round(totFail/totDone*100)+'%':'0%');set('stat-marketers',marketerSet.size.toLocaleString());set('brk-gross',__money(totGross,'£','GBP'));set('brk-comm','−'+__money(totComm,'£','GBP'));set('brk-plat','−'+__money(totPlat,'£','GBP'));set('brk-net',__money(totGross-totComm-totPlat,'£','GBP'));
...
function updateSummary(){const nc=orders.filter(o=>o.status==='new').length;const fc=orders.filter(o=>o.status==='failed').length;const st=document.getElementById('s-total');const sn=document.getElementById('s-new-sub');const ss=document.getElementById('s-ship');const sd=document.getElementById('s-done');if(st)st.textContent=orders.length;if(sn)sn.textContent=nc+' new';if(ss)ss.textContent=orders.filter(o=>o.status==='confirmed').length;if(sd)sd.textContent=orders.filter(o=>o.status==='delivered').length;const banner=document.getElementById('new-banner');if(banner)banner.style.display=nc>0?'flex':'none';const bt=document.getElementById('new-banner-text');if(bt)bt.textContent=nc+' new';const fcNew=document.getElementById('fc-new');if(fcNew)fcNew.innerHTML=nc>0?`New (${nc}) <span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#E24B4A;margin-left:3px;vertical-align:middle;animation:blink 1.5s infinite"></span>`:'New';const fcFailed=document.getElementById('fc-failed');if(fcFailed)fcFailed.textContent=fc>0?'Failed ('+fc+')':'Failed';}
...
async function advance(id,ns){const o=orders.find(x=>x.id===id);if(!o)return;let failNote=null;if(ns==='failed'){const entered=await __askFailNote();if(entered===null)return;failNote=entered||null;}try{if(window.LateenAPI&&o.dbId){if(ns==='confirmed')await window.LateenAPI.confirmOrder(o.dbId);else if(ns==='delivered')await window.LateenAPI.markDelivered(o.dbId);else if(ns==='failed')await window.LateenAPI.markFailed(o.dbId,failNote);}}catch(e){console.error('[Lateen] advance',e);alert('Action failed: '+(e.message||e));return;}await loadProducts();await loadOrders();}
function buildStepper(s){if(s==='failed')return`<div style="font-size:12px;color:#e07070;font-weight:500;margin-bottom:12px">Order failed</div>`;const si=ST[s]?.step??0;let h='<div class="stepper">';STEPS.forEach((step,i)=>{const d=i<si,c=i===si;if(i>0)h+=`<div class="stp-line ${d?'done':''}"></div>`;h+=`<div class="stp"><div class="stp-dot ${d?'done':c?'cur':''}"></div><div class="stp-lbl ${d?'done':c?'cur':''}">${step}</div></div>`;});return h+'</div>';}
function buildActions(o){if(o.status==='delivered'||o.status==='failed')return`<div class="action-row"><button class="act a-lock" disabled>${o.status==='delivered'?'✓ Delivered':'Failed'}</button></div>`;let b='';if(o.status==='new')b+=`<button class="act a-confirm" onclick="event.stopPropagation();advance('${o.id}','confirmed')">Confirm order</button>`;if(o.status==='confirmed')b+=`<button class="act a-deliver" onclick="event.stopPropagation();advance('${o.id}','delivered')">Mark delivered</button>`;if(o.status==='new'||o.status==='confirmed')b+=`<button class="act a-fail" onclick="event.stopPropagation();advance('${o.id}','failed')">Failed</button>`;return`<div class="action-row">${b}</div>`;}
function renderOrders(list){const el=document.getElementById('orders-list');if(!el)return;list=Array.isArray(list)?list:[];if(!list.length){el.innerHTML='<div class="empty-state">No orders found.</div>';return;}el.innerHTML=list.map(o=>{try{const om=n=>fmt(n,o.sym,o.curCode);const st=ST[o.status]||ST.new||{dot:'#888',label:o.status||'—'};const isExp=expandedId===o.id;const isNew=o.status==='new';const ph=o.photos&&o.photos.length;const isImg=s=>typeof s==='string'&&/^(data:|https?:|\/)/.test(s);const photoEl=`<div class="prod-photo">${ph&&isImg(o.photos[0])?`<img src="${o.photos[0]}" alt="" loading="eager" decoding="async"/>`:ph?o.photos[0]:o.productEmoji}</div>`;const strip=ph&&isExp?`<div class="photo-strip-o">${o.photos.map(p=>isImg(p)?`<div class="s-photo"><img src="${p}" alt="" loading="eager" decoding="async"/></div>`:`<div class="s-photo">${p}</div>`).join('')}</div>`:'';const rightSide=`<div class="oc-amt">${om(o.total)}</div><div class="status-dot-row"><div class="status-dot" style="background:${st.dot};opacity:0.7"></div><span class="status-text">${st.label}</span></div>`;const newTag=isNew&&!isExp?`<div class="new-tag"><div class="nd"></div></div>`:'';const payLabel=o.paymentType==='upfront'?'Upfront fee paid':'Full payment received';const codBanner=o.paymentType==='upfront'?`<div style="display:flex;align-items:center;gap:10px;background:#3a2a10;border-radius:12px;padding:11px 14px;margin-bottom:12px;"><div style="width:32px;height:32px;border-radius:8px;background:#5a3e18;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#c9923a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z"/></svg></div><div><div style="font-size:13px;font-weight:500;color:#c9923a;">Cash on delivery</div><div style="font-size:11px;color:#9e7040;margin-top:1px;">Customer pays remaining balance on arrival</div></div></div>`:'';const inner=isExp?`<div class="oc-inner open">${strip}<div class="ipad">${buildStepper(o.status)}<div class="appr-note"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>${o.marketerConfirmed?'Marketer confirmed payment · '+o.marketerConfirmedDate:'Awaiting marketer confirmation · '+o.date}</div>${o.receiptUrl?`<a href="${o.receiptUrl}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:8px;background:#1a2a3a;border-radius:10px;padding:10px 12px;margin-bottom:12px;text-decoration:none;color:#7fb3e0;font-size:12px;font-weight:500"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>View payment receipt</a>`:''}<div class="pay-badge"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>${payLabel} · ${om(o.paymentAmount)} · ${o.paymentDate}</div>${codBanner}<div class="st">Customer &amp; delivery</div><div class="db"><div class="dr"><span class="dl">Name</span><span class="dv">${o.customerName}</span></div>${(function(){var p=__splitCC(o.customerPhone);return `<div class="dr"><span class="dl">Country code</span><span class="dv" dir="ltr">${p.cc}</span></div><div class="dr"><span class="dl">Phone</span><span class="dv" dir="ltr">${p.num}</span></div>`;})()}${o.customerWhatsapp?(function(){var w=__splitCC(o.customerWhatsapp);return `<div class="dr"><span class="dl">WhatsApp code</span><span class="dv" dir="ltr">${w.cc}</span></div><div class="dr"><span class="dl">WhatsApp</span><span class="dv" dir="ltr">${w.num}</span></div>`;})():''}<div class="dr"><span class="dl">Address</span><span class="dv">${o.address}</span></div><div class="dr"><span class="dl">City</span><span class="dv">${o.city}, ${o.country}</span></div></div><div class="st">Order</div><div class="db"><div class="dr"><span class="dl">Order ID</span><span class="dv mono">${o.id}</span></div><div class="dr"><span class="dl">Source</span><span class="dv">${o.source==='manual'?'Manual (marketer)':'Affiliate link'}</span></div><div class="dr"><span class="dl">Product</span><span class="dv">${o.product}</span></div><div class="dr"><span class="dl">Code</span><span class="dv mono">${o.productCode}</span></div>${o.size?`<div class="dr"><span class="dl">Size</span><span class="dv">${o.size}</span></div>`:''}${o.color?`<div class="dr"><span class="dl">Colour</span><span class="dv">${o.color}</span></div>`:''}<div class="dr"><span class="dl">Quantity</span><span class="dv">${o.qty}</span></div></div><div class="st">Financials</div><div class="fin-grid"><div class="fi"><div class="fi-l">Product price</div><div class="fi-v">${om(o.price)}</div></div><div class="fi"><div class="fi-l">Order total</div><div class="fi-v">${om(o.total)}</div></div><div class="fi"><div class="fi-l">Shipping</div><div class="fi-v">${om(o.shipping)}</div></div><div class="fi"><div class="fi-l">Delivery</div><div class="fi-v">${om(o.delivery)}</div></div><div class="fi"><div class="fi-l">Marketer comm.</div><div class="fi-v" style="color:#e07070">−${om(o.commission)}</div></div><div class="fi"><div class="fi-l">Platform fee</div><div class="fi-v" style="color:#e07070">−${om(o.platformFee)}</div></div></div>${o.notes?`<div class="st">Notes</div><div class="notes-box">${o.notes}</div>`:''}${buildActions(o)}</div></div>`:`<div class="oc-inner"></div>`;return`<div class="oc ${isNew?'is-new':''} ${isExp?'expanded':''}" onclick="toggle('${o.id}')">${newTag}<div class="oc-outer">${photoEl}<div class="oc-left"><div class="oc-cust">${o.customerName}</div><div class="oc-sub">${o.product} · ${o.city}, ${o.country}</div></div><div class="oc-right">${rightSide}</div><svg class="chevron ${isExp?'open':''}" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg></div>${inner}</div>`;}catch(err){console.error('[Lateen] renderOrders item',err,o);return '';}}).join('');}
...
async function refreshProfile(){if(!window.LateenAPI||!window.LateenAPI.getProfile)return;try{const p=await window.LateenAPI.getProfile();window.__profileData=p;const name=(p&&p.full_name)||'';const biz=(p&&p.business_name)||'';const initials=(biz||name)?((biz||name).split(/\s+/).filter(Boolean).slice(0,2).map(s=>s[0].toUpperCase()).join('')):'··';const first=name?name.split(/\s+/)[0]:(__ar()?'يا هلا':'there');const __biz=__ar()?'التاجر':'Business';const __welcome=__ar()?'أهلاً بك':'Welcome';const av=p&&p.avatar_signed_url;__avCache(av||'');['user-avatar','user-avatar-p','user-avatar-o','menu-avatar'].forEach(id=>__applyAvatarTo(document.getElementById(id),av,initials));const g=document.getElementById('user-greet');if(g)g.textContent=(__ar()?'هلا، ':'Hey, ')+first;const s=document.getElementById('user-sub');if(s)s.textContent=biz||__biz;const mn=document.getElementById('menu-name');if(mn)mn.textContent=name||__welcome;const ms=document.getElementById('menu-sub');if(ms)ms.textContent=(biz?biz+' · ':'')+__biz;}catch(e){console.error('[Lateen] profile',e);}}
function __profT(){const ar=__ar();return{title:ar?'الملف الشخصي':'Profile',name:ar?'الاسم الكامل':'Full name',biz:ar?'اسم التاجر':'Business name',phone:ar?'رقم الهاتف':'Phone number',email:ar?'البريد الإلكتروني':'Email',wa:ar?'واتساب':'WhatsApp',opt:ar?'(اختياري)':'(optional)',save:ar?'حفظ':'Save',saving:ar?'جارٍ الحفظ…':'Saving…',saved:ar?'تم الحفظ':'Saved',hint:ar?'اضغط على أيقونة الكاميرا لتغيير الصورة':'Tap the camera to change photo',tooBig:ar?'الصورة كبيرة جداً (أقصى 5 ميغابايت)':'Image too large (max 5 MB)',uploading:ar?'جارٍ رفع الصورة…':'Uploading photo…'};}
