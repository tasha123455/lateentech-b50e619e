...
function __moneyH(n,sym,code){const a=parseFloat(n||0).toFixed(2);const cc=(code||__SYM2CODE[__stripDirMarks(sym||'')]||'').toString().toUpperCase();const r=__stripDirMarks(__rawSym(sym||'£',cc));const isAr=(typeof document!=='undefined'&&document.documentElement.lang==='ar');const symHtml='<span class="cur-sym">'+__escH(r)+'</span>';const codeHtml=cc?'<span class="cur-sym">'+__escH(cc)+'</span>':'';return isAr?(symHtml+__escH(a)):(cc?(__escH(a)+' '+codeHtml):(symHtml+__escH(a)));}
function __priceSym(cur){if(!cur)return'—';const isAr=(typeof document!=='undefined'&&document.documentElement.lang==='ar');return(cur.code==='LYD')?(isAr?'د.ل':'LYD'):cur.symbol;}
...
function platformFeeForPrice(price){const pr=Number(price)||0;return pr>PLATFORM_FEE_THRESHOLD?parseFloat((pr*PLATFORM_FEE_RATE).toFixed(2)):PLATFORM_FEE_FIXED;}
...
document.addEventListener('click',(e)=>{const picker=document.getElementById('category-picker');if(picker&&categoryPanelOpen&&!picker.contains(e.target))closeCategoryPanel();});
 ...
 let photos=[],variantGroups=[],zones={},editingId=null,commMode='pct',currentCode='',variantIdCounter=0,groupIdCounter=0,selectedCurrency=null,currDropdownOpen=false,countryDropdownOpen=false,selectedCountry=null,__photoUploadJobs=[],__variantPhotoUploadJobs=[],variantMode='none',simpleQty='',cityPanelOpenFor=null,coverFocusX=50,coverFocusY=50,__lastCoverUrl=null,__coverDragInited=false,__editLocked=false,__editLockActiveCount=0;
...
let products=[];
let productRequireAdditionalPhone=false;

/* ── Backend bridge: map DB rows to the in-memory shape this UI expects ── */
function dbToProduct(r){const cur=r.currency?{...r.currency,symbol:__wrapArSym(r.currency.symbol,r.currency.code)}:null;return{id:r.id,code:r.code,bizName:r.biz_name||'',currency:cur,photos:r.photos||[],coverFocusX:(r.cover_focus_x!=null&&!isNaN(Number(r.cover_focus_x)))?Number(r.cover_focus_x):50,coverFocusY:(r.cover_focus_y!=null&&!isNaN(Number(r.cover_focus_y)))?Number(r.cover_focus_y):50,name:r.name,desc:r.description||'',price:Number(r.price)||0,costPrice:Number(r.cost_price)||0,commPct:Number(r.comm_pct)||0,commFixed:Number(r.comm_fixed)||0,commMode:r.comm_mode||'pct',platformFee:Number(r.platform_fee)||0,totalFeePerUnit:Number(r.total_fee_per_unit)||0,qty:Number(r.qty)||0,category:r.category||'',sizes:r.sizes||[],colors:r.colors||[],variantGroups:r.variant_groups||[],sold:Number(r.sold)||0,revenue:Number(r.revenue)||0,status:r.status||'active',delivery:r.delivery||{},reqPhone:!!r.require_additional_phone};}
...
function selectCurrency(code){selectedCurrency=CURRENCIES.find(c=>c.code===code);document.getElementById('curr-flag').textContent=selectedCurrency.flag;document.getElementById('curr-name').textContent=selectedCurrency.name+' ('+selectedCurrency.code+')';document.getElementById('curr-symbol-label').textContent=selectedCurrency.symbol;document.getElementById('price-prefix').textContent=__priceSym(selectedCurrency);updateCommFixedUnitLabel();currDropdownOpen=false;document.getElementById('curr-dropdown').classList.remove('open');document.getElementById('curr-search-wrap').style.display='none';document.getElementById('curr-search').value='';updateFeeBreakdown();}
function calcFees(price,pct,fixedComm,mode){const comm=mode==='pct'?pctOf(price,pct):parseFloat(fixedComm||0);const platform=platformFeeForPrice(price);const total=parseFloat((comm+platform).toFixed(2));const commPct=mode==='pct'?pct:parseFloat(((comm/price)*100).toFixed(1));return{comm,platform,total,commPct};}
...
function onPhotosSelected(input){const files=Array.from(input.files);input.value='';const slots=[];for(const file of files){if(photos.length>=6)break;const localUrl=URL.createObjectURL(file);photos.push(localUrl);slots.push({file,localUrl});}renderPhotoGrid();const job=(async()=>{await Promise.all(slots.map(async s=>{try{if(window.LateenAPI&&window.LateenAPI.uploadPhoto){const url=await window.LateenAPI.uploadPhoto(s.file);const curIdx=photos.indexOf(s.localUrl);if(curIdx!==-1)photos[curIdx]=url;}else{const url=await new Promise((res,rej)=>{const r=new FileReader();r.onload=e=>res(e.target.result);r.onerror=rej;r.readAsDataURL(s.file);});const curIdx=photos.indexOf(s.localUrl);if(curIdx!==-1)photos[curIdx]=url;}try{URL.revokeObjectURL(s.localUrl);}catch(e){}}catch(e){console.error('[Lateen] uploadPhoto',e);const curIdx=photos.indexOf(s.localUrl);if(curIdx!==-1)photos.splice(curIdx,1);alert(__ar()?'فشل رفع الصورة.':'Photo upload failed.');}}));renderPhotoGrid();})();__photoUploadJobs.push(job);job.finally(()=>{__photoUploadJobs=__photoUploadJobs.filter(j=>j!==job);});}
function removePhoto(i){photos.splice(i,1);renderPhotoGrid();}
function applyCoverFocus(){const img=document.getElementById('lp-cover-img');if(img)img.style.objectPosition=coverFocusX+'% '+coverFocusY+'%';}
function initCoverDrag(){const frame=document.getElementById('lp-cover-frame');if(!frame)return;let dragging=false,pid=null,startX=0,startY=0,startFx=50,startFy=50;function onDown(e){if(!photos.length)return;if(e.target&&e.target.closest&&e.target.closest('.lp-cover-remove'))return;dragging=true;pid=e.pointerId;startX=e.clientX;startY=e.clientY;startFx=coverFocusX;startFy=coverFocusY;try{frame.setPointerCapture(pid);}catch(err){}e.preventDefault();}function onMove(e){if(!dragging||e.pointerId!==pid)return;const r=frame.getBoundingClientRect();if(!r.width||!r.height)return;const dx=((e.clientX-startX)/r.width)*100;const dy=((e.clientY-startY)/r.height)*100;coverFocusX=Math.max(0,Math.min(100,startFx-dx));coverFocusY=Math.max(0,Math.min(100,startFy-dy));applyCoverFocus();}function onUp(){if(pid!=null){try{frame.releasePointerCapture(pid);}catch(err){}}dragging=false;pid=null;}frame.addEventListener('pointerdown',onDown);frame.addEventListener('pointermove',onMove);frame.addEventListener('pointerup',onUp);frame.addEventListener('pointercancel',onUp);window.__lateenUnsubs=window.__lateenUnsubs||[];window.__lateenUnsubs.push(()=>{frame.removeEventListener('pointerdown',onDown);frame.removeEventListener('pointermove',onMove);frame.removeEventListener('pointerup',onUp);frame.removeEventListener('pointercancel',onUp);});}
function renderPhotoGrid(){const frame=document.getElementById('lp-cover-frame');const placeholder=document.getElementById('lp-cover-placeholder');const img=document.getElementById('lp-cover-img');const removeBtn=document.getElementById('lp-cover-remove');const caption=document.getElementById('lp-cover-caption');const coverUrl=photos[0]||null;if(coverUrl!==__lastCoverUrl){coverFocusX=50;coverFocusY=50;__lastCoverUrl=coverUrl;}if(img&&placeholder){if(coverUrl){placeholder.style.display='none';img.style.display='block';img.src=coverUrl;img.style.objectPosition=coverFocusX+'% '+coverFocusY+'%';if(removeBtn)removeBtn.style.display='flex';if(caption)caption.style.display='block';}else{placeholder.style.display='flex';img.style.display='none';img.removeAttribute('src');if(removeBtn)removeBtn.style.display='none';if(caption)caption.style.display='none';}}if(frame&&!__coverDragInited){initCoverDrag();__coverDragInited=true;}const g=document.getElementById('photo-grid');if(!g)return;const rest=photos.slice(1);g.innerHTML=rest.map((src,i)=>`<div class="photo-preview"><img src="${src}"/><button class="photo-remove" onclick="removePhoto(${i+1})">×</button></div>`).join('')+(photos.length<6?`<div class="add-photo-btn" onclick="document.getElementById('photo-input').click()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--color-text-secondary)" stroke-width="1.6" stroke-linecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg><div class="add-photo-label">Add photo</div></div>`:'');}
...
function __stepVariantQty(gid,iid,d){const g=variantGroups.find(x=>x.id===gid);if(!g)return;const it=g.items.find(x=>x.id===iid);if(!it)return;const cur=Math.max(0,parseInt(String(it.qty||'0'),10)||0);const nv=Math.max(0,cur+d);it.qty=nv;const inp=document.getElementById('vb-qty-'+gid+'-'+iid);if(inp)inp.value=String(nv);updateVariantTotalStock();}
function __onVariantQtyInput(gid,iid,el){let v=parseInt(el.value||'',10);if(!Number.isFinite(v)||v<0)v=0;el.value=String(v);updateVariantQty(gid,iid,String(v));}
...
function addVariant(gid){const g=variantGroups.find(x=>x.id===gid);if(!g)return;g.items.push({id:variantIdCounter++,val:'',qty:0,photo:''});renderVariantGroups();setTimeout(()=>{const ins=document.querySelectorAll(`#vg-list-${gid} input.vb-val-input`);if(ins.length)ins[ins.length-1].focus();},50);}
...
function toggleAllCities(code){if(!zones[code])return;const scroller=document.getElementById('city-panel-'+code);const savedScroll=scroller?scroller.scrollTop:0;const all=LIBYA_CITIES.every(c=>!!zones[code].cities[c.en]);if(all){LIBYA_CITIES.forEach(c=>{delete zones[code].cities[c.en];});}else{LIBYA_CITIES.forEach(c=>{if(!zones[code].cities[c.en])zones[code].cities[c.en]={delivery:''};});}renderZoneBuilderList();const r=document.getElementById('city-panel-'+code);if(r)r.scrollTop=savedScroll;}
function removeCity(code,city){delete zones[code].cities[city];renderZoneBuilderList();}
function updateCityCost(code,city,field,val){if(zones[code]&&zones[code].cities[city]){const n=parseFloat(val);zones[code].cities[city][field]=(val===''||val==null||isNaN(n))?'':(n<0?0:n);}renderZoneBuilderList();}
function updateZoneShipping(code,val){if(zones[code]){const n=parseFloat(val);zones[code].shipping=(val===''||val==null||isNaN(n))?'':(n<0?0:n);}renderZoneBuilderList();}
function __freeLbl(){return (typeof __ar==='function'&&__ar())?'مجاني':'Free';}
function __isFreeVal(v){return v!==''&&v!=null&&Number(v)===0;}
function renderZoneBuilderList(){const el=document.getElementById('zone-builder-list');const keys=Object.keys(zones);if(!keys.length){el.innerHTML='';return;}const ar=(typeof __ar==='function'&&__ar());const shipLblFor=(name)=>ar?`تكلفة الشحن لـ ${name} (اختياري)`:`Shipping cost for ${name} (optional)`;const cityLbl=ar?'المدينة':'City';const dlvLbl=ar?'تكلفة التوصيل':'Delivery cost';el.innerHTML=keys.map(code=>{const z=zones[code];const ck=Object.keys(z.cities);const isLY=code==='LY';const countryMeta=COUNTRIES.find(c=>c.code===code);const flag=countryMeta?countryMeta.flag:'\ud83c\udf10';const cname=__countryLbl(code);let adder;if(isLY){const isOpen=cityPanelOpenFor===code;const placeholder=ck.length?(ar?'+ إضافة مدينة':'+ Add city'):(ar?'اختر مدينة…':'Select a city…');const doneLbl=ar?'تم ✓':'Done ✓';const toggleBtn=`<div onclick="toggleCityPanel('${code}')" style="width:100%;padding:11px 12px;font-size:13px;border:1.5px dashed var(--color-border-secondary);border-radius:12px;background:transparent;color:#34c77b;font-weight:700;font-family:var(--font-sans);margin-top:10px;text-align:center;cursor:pointer;">${placeholder}</div>`;let panel='';if(isOpen){const rows=LIBYA_CITIES.map(c=>{const checked=!!z.cities[c.en];const safeEn=c.en.replace(/'/g,"\\'").replace(/"/g,'&quot;');return`<label style="display:flex;align-items:center;gap:10px;padding:9px 2px;cursor:pointer;border-bottom:0.5px solid var(--color-border-tertiary);"><input type="checkbox" ${checked?'checked':''} onchange="toggleCityCheck('${code}','${safeEn}')" style="width:18px;height:18px;accent-color:#34c77b;flex-shrink:0;"/><span style="font-size:13px;color:var(--color-text-primary);">${__cityLbl(c.en)}</span></label>`;}).join('');const allChecked=LIBYA_CITIES.every(c=>!!z.cities[c.en]);const allRow=`<label style="display:flex;align-items:center;gap:10px;padding:10px 12px;cursor:pointer;border-bottom:0.5px solid var(--color-border-secondary);background:var(--color-background-secondary);"><input type="checkbox" ${allChecked?'checked':''} onchange="toggleAllCities('${code}')" style="width:18px;height:18px;accent-color:#34c77b;flex-shrink:0;"/><span style="font-size:13px;font-weight:700;color:var(--color-text-primary);">${ar?'الكل':'All'}</span></label>`;panel=`<div style="margin-top:10px;border:0.5px solid var(--color-border-secondary);border-radius:12px;overflow:hidden;background:var(--color-background-primary);"><div onclick="toggleCityPanel('${code}')" style="padding:9px 12px;text-align:center;font-size:12px;font-weight:700;color:#34c77b;background:var(--color-background-secondary);cursor:pointer;">${doneLbl}</div>${allRow}<div id="city-panel-${code}" style="max-height:260px;overflow-y:auto;padding:2px 12px;">${rows}</div></div>`;}adder=toggleBtn+panel;}else{adder=`<div style="display:flex;gap:6px;margin-top:10px"><input id="new-city-${code}" type="text" placeholder="${ar?'أضف مدينة…':'Add a city…'}" style="flex:1;padding:9px 10px;font-size:13px;border:0.5px solid var(--color-border-secondary);border-radius:10px;background:var(--color-background-primary);color:var(--color-text-primary);outline:none;font-family:var(--font-sans);" onkeydown="if(event.key==='Enter'){event.preventDefault();addCity('${code}');}"/><button onclick="addCity('${code}')" style="height:38px;padding:0 14px;border-radius:10px;border:none;background:#34c77b;color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:var(--font-sans);">${ar?'إضافة':'Add'}</button></div>`;}const shipRow=`<div class="lp-cost-row"><span class="cost-label">${shipLblFor(cname)}</span><input type="number" min="0" value="${z.shipping===''||z.shipping==null?'':z.shipping}" placeholder="0.00" step="0.01" onchange="updateZoneShipping('${code}',this.value)"/>${__isFreeVal(z.shipping)?`<span style="margin-inline-start:8px;font-size:11px;font-weight:700;color:#34c77b;">${__freeLbl()}</span>`:''}</div>`;const cityRows=ck.map(city=>`<div class="lp-city-row-flex"><span class="city-name">${isLY?__cityLbl(city):city} <span class="x-mark" onclick="removeCity('${code}','${city}')">✕</span></span><input type="number" min="0" value="${z.cities[city].delivery===''||z.cities[city].delivery==null?'':z.cities[city].delivery}" placeholder="0.00" step="0.01" onchange="updateCityCost('${code}','${city}','delivery',this.value)"/>${__isFreeVal(z.cities[city].delivery)?`<span style="margin-inline-start:8px;font-size:11px;font-weight:700;color:#34c77b;">${__freeLbl()}</span>`:''}</div>`).join('');return`<div class="lp-country-card-2"><div class="lp-country-card-head"><span>${flag} ${cname}</span><span class="rm" onclick="removeZone('${code}')">✕</span></div>${shipRow}${ck.length?`<div class="lp-city-values-label"><span>${cityLbl}</span><span>${dlvLbl}</span></div>${cityRows}`:''}${adder}</div>`;}).join('');}
...
function openAddForm(p){if(!p){__editLocked=false;__editLockActiveCount=0;}productRequireAdditionalPhone=!!(p&&p.reqPhone);photos=p?[...(p.photos||[])]:[];coverFocusX=(p&&p.coverFocusX!=null)?Number(p.coverFocusX):50;coverFocusY=(p&&p.coverFocusY!=null)?Number(p.coverFocusY):50;__lastCoverUrl=photos[0]||null;variantGroups=hydrateVariantGroups(p);variantMode=variantGroups.length?'variants':'none';simpleQty=(p&&p.qty!=null&&Number(p.qty)>=0)?String(Math.round(Number(p.qty))):'0';zones=p?JSON.parse(JSON.stringify(p.delivery)):{};Object.keys(zones).forEach(code=>{const z=zones[code];if(!z.cities)z.cities={};let s=0;Object.values(z.cities).forEach(c=>{const cs=Number(c.shipping)||0;if(cs>s)s=cs;});if(z.shipping==null||z.shipping==='')z.shipping=s||'';});commMode=p?.commMode||'pct';selectedCurrency=p?.currency||__defaultCurrencyForCountry();currentCode=ensureUniqueCode(p?.code);document.getElementById('product-code-display').textContent=currentCode;document.getElementById('form-title').textContent=p?'Edit product':'Add a product';document.getElementById('f-name').value=p?.name||'';document.getElementById('f-desc').value=p?.desc||'';document.getElementById('f-price').value=p?.price||'';document.getElementById('f-cost').value=p?.costPrice||'';selectedCategory=p?.category||'';const catVal=document.getElementById('category-picker-value');if(selectedCategory){catVal.textContent=__ar()?(CATEGORY_ITEM_AR[selectedCategory]||selectedCategory):selectedCategory;catVal.classList.remove('placeholder');}else{catVal.textContent='Select category';catVal.classList.add('placeholder');}document.getElementById('f-comm-pct').value=p?.commPct||'';document.getElementById('f-comm-fixed').value=p?.commFixed||'';if(selectedCurrency){document.getElementById('curr-flag').textContent=selectedCurrency.flag;document.getElementById('curr-name').textContent=selectedCurrency.name+' ('+selectedCurrency.code+')';document.getElementById('curr-symbol-label').textContent=selectedCurrency.symbol;document.getElementById('price-prefix').textContent=__priceSym(selectedCurrency);}else{document.getElementById('curr-flag').textContent='🌐';document.getElementById('curr-name').textContent='Select a currency';document.getElementById('curr-symbol-label').textContent='';document.getElementById('price-prefix').textContent='—';}updateCommFixedUnitLabel();setCommMode(commMode);renderPhotoGrid();renderVariantGroups();setVariantMode(variantMode);{const __sq=document.getElementById('f-simple-qty');if(__sq){const __v=parseInt(simpleQty||'0',10);__sq.value=String(!Number.isFinite(__v)||__v<0?0:__v);}}renderZoneBuilderList();updateConversionNote();updateProductSummaryCard();__applyReqPhoneToggleUI();applyProductEditLock();document.getElementById('form-overlay').classList.add('open');}
function closeForm(){document.getElementById('form-overlay').classList.remove('open');editingId=null;photos=[];coverFocusX=50;coverFocusY=50;__lastCoverUrl=null;variantGroups=[];variantMode='none';simpleQty='';zones={};selectedCurrency=null;selectedCategory='';productRequireAdditionalPhone=false;currDropdownOpen=false;countryDropdownOpen=false;cityPanelOpenFor=null;closeCategoryPanel();__editLocked=false;__editLockActiveCount=0;const __elb=document.getElementById('edit-lock-banner');if(__elb){__elb.style.display='none';__elb.textContent='';}}
function applyProductEditLock(){
  const locked=!!__editLocked;
  const n=__editLockActiveCount||0;
  const sheet=document.querySelector('#form-overlay .sheet');
  if(sheet){
    sheet.querySelectorAll('input,textarea,select,button').forEach(el=>{
      if(el.classList.contains('lp-btn-secondary'))return; // always allow Cancel
      el.disabled=locked;
    });
    ['curr-selected','category-picker-input'].forEach(id=>{const el=document.getElementById(id);if(el){el.style.pointerEvents=locked?'none':'';el.style.opacity=locked?'0.55':'';el.style.cursor=locked?'not-allowed':'';}});
    const uploadBox=sheet.querySelector('.lp-upload-box');
  ...
  if(banner){
    if(locked){
      const ar=__ar();
      const label=ar?(n===1?'مسوّق نشط واحد':(n===2?'مسوّقين نشطين':n+' مسوّقين نشطين')):(n+' active marketer'+(n===1?'':'s'));
      banner.textContent=ar?`🔒 هذا المنتج مقفل بالكامل — لديه ${label} حالياً، لذلك لا يمكن تعديله أو حذفه حتى تكتمل طلباتهم.`:`🔒 This product is fully locked — it has ${label} right now, so it can't be edited or deleted until those orders complete.`;
...
function mpSlidesHtml(p){
  ...
  return photos.map(url=>`<div class="mp-p-thumb-slide"><img src="${mpEsc(url)}" alt="${mpEsc(p.name)}" data-no-i18n loading="eager" decoding="async" onerror="this.onerror=null;this.parentElement.classList.add('mp-p-thumb-empty');this.replaceWith(Object.assign(document.createElement('span'),{textContent:'📦'}));"/></div>`).join('');
}
  ...
  if(realGroups.length){
    return realGroups.map(g=>{
      ...
      const rows=items.map(x=>{
        ...
        return `<div class="mp-vg-value-row">
          <div class="mp-vg-value-top">
            ${swatch}
            <span class="mp-vg-value-name" data-no-i18n>${mpEsc(x.val)}</span>
      ...
      return `<div class="mp-variant-group-box">
        <div class="mp-vg-title-row"><span class="mp-vg-title">${g.name?`<span data-no-i18n>${mpEsc(g.name)}</span>`:'Variant'}</span><span class="mp-vg-total">${total} ${__ar()?'بالمخزون':'in stock'}</span></div>
        ${rows}
      </div>`;
    }).join('');
  }
...
function mpReviewsFold(p){
  ...
  const body=list.length
    ?list.map(r=>{const stars=Math.max(0,Math.min(5,Number(r.rating)||0));const initials=(r.author||'M').trim().charAt(0).toUpperCase();const ava=r.avatar?`<span class="avatar" style="width:20px;height:20px;font-size:9px;flex-shrink:0;overflow:hidden;padding:0"><img src="${mpEsc(r.avatar)}" alt="" style="width:100%;height:100%;object-fit:cover;object-position:center;display:block;border-radius:inherit;" onerror="this.parentElement.style.background='#0A3C2A';this.parentElement.textContent='${mpEsc(initials)}';"/></span>`:`<span class="avatar" style="width:20px;height:20px;font-size:9px;flex-shrink:0;">${mpEsc(initials)}</span>`;const photoHtml=r.photo?`<div style="margin-top:6px;width:56px;height:56px;border-radius:10px;overflow:hidden;cursor:pointer;border:1px solid var(--mp-border);" onclick="event.stopPropagation();mpOpenLightbox('${String(r.photo).replace(/'/g,"\\'")}')"><img src="${mpEsc(r.photo)}" alt="" style="width:100%;height:100%;object-fit:cover;display:block;"/></div>`:'';return `<div class="mp-review-item"><div class="mp-review-top"><span style="display:flex;align-items:center;gap:6px;min-width:0;">${ava}<span class="mp-review-name">${r.author?`<span data-no-i18n>${mpEsc(r.author)}</span>`:'Marketer'}</span></span><span class="mp-review-stars">${'★'.repeat(stars)}${'☆'.repeat(5-stars)}</span></div>${r.text?`<div class="mp-review-text" data-no-i18n>${mpEsc(r.text)}</div>`:''}${photoHtml}</div>`;}).join('')
...
function mpActionsRow(p){
  ...
  const __n=mpActiveMarketerCount(p);
  const __locked=__n>0;
  const __ar3=__ar();
  const __delTitle=__locked?(__ar3?`لا يمكن الحذف — ${__n} ${__n===1?'مسوّق نشط':'مسوّقين نشطين'}`:`Can't delete — ${__n} active marketer${__n===1?'':'s'}`):(__ar3?'حذف':'Delete');
  return `<div class="mp-action-row">
    ...
    <div class="mp-icon-btn mp-danger"${__locked?' style="opacity:.4"':''} title="${__delTitle}" onclick="event.stopPropagation();deleteProduct('${p.id}')">${MP_ICON_TRASH}</div>
  </div>`;
}
  ...
  return `<div class="mp-product-card" data-id="${p.id}">
    <div class="mp-p-head" onclick="mpToggleCard('${p.id}')">
      ...
      <div class="mp-p-code-corner">
        ...
        <span class="mp-code-val" data-no-i18n>${mpEsc(p.code)}</span>
      </div>
...
async function submitProduct(){if(__submittingProduct)return;if(__frozenBlock())return;const btn=document.querySelector('#form-overlay .submit-btn');const name=document.getElementById('f-name').value.trim();const price=parseFloat(document.getElementById('f-price').value)||0;const costPrice=parseFloat(document.getElementById('f-cost').value)||0;const commPct=parseFloat(document.getElementById('f-comm-pct').value)||0;const commFixed=parseFloat(document.getElementById('f-comm-fixed').value)||0;console.log('[Lateen] submitProduct fired',{name,price,costPrice,commPct,commFixed,selectedCurrency,zones});if(!name||!price||(!commPct&&!commFixed)){alert(__ar()?'يرجى إدخال الاسم والسعر والعمولة.':'Please fill in name, price and commission.');return;}if(!document.getElementById('f-desc').value.trim()){alert(__ar()?'يرجى إدخال وصف المنتج.':'Please add a product description.');return;}if(!selectedCategory){alert(__ar()?'يرجى اختيار فئة للمنتج.':'Please select a category.');return;}if(!photos.length){alert(__ar()?'يرجى إضافة صورة واحدة على الأقل للمنتج.':'Please add at least one product photo.');return;}if(!selectedCurrency){alert(__ar()?'يرجى اختيار عملة.':'Please select a currency.');return;}if(variantMode==='none'){if(simpleQty===''||simpleQty==null||isNaN(Number(simpleQty))||Number(simpleQty)<0){alert(__ar()?'يرجى إدخال كمية صحيحة.':'Please enter a valid quantity.');return;}}else{for(const __vg of variantGroups){const __vgName=(__vg.name||'').trim();const __vgItems=__vg.items||[];const __vgBlank=__vgItems.filter(__it=>!(__it.val||'').toString().trim());const __vgFilled=__vgItems.filter(__it=>(__it.val||'').toString().trim());const __vgIsEmpty=!__vgName&&!__vgFilled.length;if(__vgIsEmpty)continue;if(__vgBlank.length){alert(__ar()?'يرجى تعبئة قيمة لكل خيار تضيفه، أو احذف الخيار الفارغ.':'Please fill in a value for every variant you add, or remove the empty one.');return;}if(!__vgFilled.length){alert(__ar()?('يرجى إضافة قيمة واحدة على الأقل لخيار "'+(__vgName||'الخيار')+'"، أو احذفه.'):('Please add at least one value for the "'+(__vgName||'variant')+'" variant, or remove it.'));return;}const __vgNoQty=__vgFilled.filter(__it=>__it.qty===''||__it.qty==null||isNaN(Number(__it.qty)));if(__vgNoQty.length){alert(__ar()?'يرجى إدخال الكمية لكل قيمة.':'Please enter a quantity for every variant value.');return;}}}if(!Object.keys(zones).length){alert(__ar()?'أضف منطقة توصيل واحدة على الأقل.':'Please add at least one delivery zone.');return;}const validZones={};for(const[code,z]of Object.entries(zones)){const zShip=Number(z.shipping)||0;const vc={};for(const[city,costs]of Object.entries(z.cities||{})){const raw=costs?costs.delivery:'';if(raw===''||raw==null||isNaN(Number(raw)))continue;const dlv=Math.max(0,Number(raw));vc[city]={shipping:zShip,delivery:dlv};}if(Object.keys(vc).length)validZones[code]={cities:vc,shipping:zShip};}if(!Object.keys(validZones).length){alert(__ar()?'أضف مدينة واحدة على الأقل مع سعر التوصيل.':'Please add at least one city with a delivery price.');return;}currentCode=ensureUniqueCode(editingId?currentCode:null);__submittingProduct=true;if(btn){btn.disabled=true;btn.style.opacity='0.6';btn.style.pointerEvents='none';btn.dataset.origText=btn.textContent;btn.textContent='Uploading photos…';}try{await __waitForProductPhotoUploads();const{comm,platform,total,commPct:finalPct}=calcFees(price,commPct,commFixed,commMode);const cleanGroups=variantMode==='variants'?variantGroups.map(g=>({name:(g.name||'Variant').trim()||'Variant',items:g.items.map(x=>({val:(x.val||'').trim(),qty:Number(x.qty)||0,photo:(typeof x.photo==='string'&&!__isBlobUrl(x.photo))?x.photo:''})).filter(x=>x.val)})).filter(g=>g.items.length):[];const sizesGroup=cleanGroups.find(g=>/size/i.test(g.name));const colorsGroup=cleanGroups.find(g=>/colou?r/i.test(g.name));const qty=variantMode==='variants'?(cleanGroups.length?Math.min(...cleanGroups.map(g=>g.items.reduce((s2,x)=>s2+(Number(x.qty)||0),0))):0):(Number(simpleQty)||0);const cleanPhotos=(photos||[]).filter(u=>typeof u==='string'&&!__isBlobUrl(u));const payload={code:currentCode,name,description:document.getElementById('f-desc').value.trim(),category:selectedCategory,price,cost_price:costPrice,qty,currency:selectedCurrency,comm_pct:finalPct,comm_fixed:comm,comm_mode:commMode,platform_fee:platform,total_fee_per_unit:total,variant_groups:cleanGroups,sizes:sizesGroup?sizesGroup.items.map(x=>x.val):[],colors:colorsGroup?colorsGroup.items.map(x=>x.val):[],delivery:validZones,photos:cleanPhotos,cover_focus_x:coverFocusX,cover_focus_y:coverFocusY,biz_name:(typeof BUSINESS!=='undefined'?BUSINESS.name:null),require_additional_phone:productRequireAdditionalPhone};if(editingId)payload.id=editingId;console.log('[Lateen] payload ready',payload);if(btn)btn.textContent='Saving…';let __codeLen=6,__saveAttempt=0,__saved=false;while(!__saved){try{await window.LateenAPI.upsertProduct(payload);__saved=true;}catch(__err){const __isDupCode=!editingId&&__err&&(__err.code==='23505'||/duplicate key|unique constraint/i.test(__err.message||''));if(__isDupCode&&__saveAttempt<8){__saveAttempt++;if(__saveAttempt>=4)__codeLen=7;currentCode=genCode(__codeLen);payload.code=currentCode;const __cd=document.getElementById('product-code-display');if(__cd)__cd.textContent=currentCode;continue;}throw __err;}}closeForm();await loadProducts();}catch(e){console.error('[Lateen] upsertProduct',e);alert((__ar()?'تعذّر حفظ المنتج: ':'Could not save product: ')+(e.message||e));}finally{__submittingProduct=false;if(btn){btn.disabled=false;btn.style.opacity='';btn.style.pointerEvents='';if(btn.dataset.origText){btn.textContent=btn.dataset.origText;delete btn.dataset.origText;}}}}
function __showPauseBlockedCard(){const ar=__ar();const title=ar?'تعذر إيقاف المنتج مؤقتا':'Unable to Pause Product';const body=ar?'هذا المنتج فيه طلبات نشطة من المسوقين، أول ما تتصفى كل الطلبات تقدر توقف المنتج.':'This product has active marketer orders. You can pause it once all orders have been cleared.';const ok=ar?'حسناً':'Got it';const ov=document.createElement('div');ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;';ov.innerHTML=`<div style=\"background:#1a2030;border-radius:16px;padding:18px;width:100%;max-width:380px;box-shadow:0 10px 40px rgba(0,0,0,0.5)\"><div style=\"font-size:15px;font-weight:700;color:#fff;margin-bottom:8px\">${title}</div><div style=\"font-size:13px;color:#c7ccd6;line-height:1.6\">${body}</div><div style=\"display:flex;justify-content:flex-end;margin-top:16px\"><button id=\"__pause_blk_ok\" style=\"background:#34c77b;color:#0f1420;border:0;border-radius:10px;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer\">${ok}</button></div></div>`;document.body.appendChild(ov);const cleanup=()=>{try{document.body.removeChild(ov);}catch(_){}};ov.querySelector('#__pause_blk_ok').onclick=cleanup;ov.addEventListener('click',ev=>{if(ev.target===ov)cleanup();});}
function __showAdminHiddenCard(){const ar=__ar();const title=ar?'المنتج مخفي من الإدارة':'Hidden by the administrator';const body=ar?'هذا المنتج تم إخفاؤه من قبل الإدارة، ولا يمكن إعادة تفعيله إلا من الإدارة.':'This product was hidden by an administrator. Only an administrator can make it active again.';const ok=ar?'حسناً':'Got it';const ov=document.createElement('div');ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;';ov.innerHTML=`<div style="background:#1a2030;border-radius:16px;padding:18px;width:100%;max-width:380px;box-shadow:0 10px 40px rgba(0,0,0,0.5)"><div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:8px">${title}</div><div style="font-size:13px;color:#c7ccd6;line-height:1.6">${body}</div><div style="display:flex;justify-content:flex-end;margin-top:16px"><button id="__adm_hid_ok" style="background:#34c77b;color:#0f1420;border:0;border-radius:10px;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer">${ok}</button></div></div>`;document.body.appendChild(ov);const cleanup=()=>{try{document.body.removeChild(ov);}catch(_){}};ov.querySelector('#__adm_hid_ok').onclick=cleanup;ov.addEventListener('click',ev=>{if(ev.target===ov)cleanup();});}
async function toggleStatus(id){const p=products.find(x=>x.id===id);if(!p)return;if(p.status==='hidden'){__showAdminHiddenCard();return;}const next=p.status==='active'?'paused':'active';if(next==='paused'){const __n=mpActiveMarketerCount(p);if(__n>0){try{await window.LateenAPI.requestPauseProduct(id);}catch(e){console.error(e);}__showPauseBlockedCard();return;}}try{await window.LateenAPI.setStatus(id,next);await loadProducts();}catch(e){console.error(e);if((e&&e.message||'').indexOf('ADMIN_HIDDEN')>=0){__showAdminHiddenCard();return;}alert(__ar()?'فشل تحديث الحالة':'Failed to update status');}}

function __applyReqPhoneToggleUI(){const btn=document.getElementById('require-wa-toggle');if(!btn)return;btn.classList.toggle('on',!!productRequireAdditionalPhone);btn.setAttribute('aria-checked',productRequireAdditionalPhone?'true':'false');}
function onToggleProductReqPhone(){productRequireAdditionalPhone=!productRequireAdditionalPhone;__applyReqPhoneToggleUI();}
function openRequireAdditionalPhoneInfo(e){if(e){e.preventDefault();e.stopPropagation();}const ar=__ar();const title=ar?'طلب رقم هاتف إضافي من الزبون':'Require Additional Phone Number from customer';const body=ar?'عند تفعيل هذا الخيار، لازم المسوق يدخل رقم هاتف إضافي من الزبون قبل تسجيل الطلب. هذا يساعدك في التواصل مع الزبون برقم آخر عند الحاجة. إذا كان الخيار غير مفعّل، يبقى الرقم الإضافي اختياري، ويتم تسجيل الطلب برقم واحد فقط من الزبون.':'When this option is enabled, marketers must provide an additional phone number from the customer before submitting an order. This helps you contact the customer using an alternative number when needed. If this option is disabled, the additional phone number remains optional, and only one phone number will be mandatory from the customer.';const ok=ar?'حسناً':'Got it';const ov=document.createElement('div');ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;';ov.innerHTML=`<div style="background:#1a2030;border-radius:16px;padding:18px;width:100%;max-width:380px;box-shadow:0 10px 40px rgba(0,0,0,0.5)"><div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:8px">${title}</div><div style="font-size:13px;color:#c7ccd6;line-height:1.6">${body}</div><div style="display:flex;justify-content:flex-end;margin-top:16px"><button id="__reqwa_ok" style="background:#34c77b;color:#0f1420;border:0;border-radius:10px;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer">${ok}</button></div></div>`;document.body.appendChild(ov);const cleanup=()=>{try{document.body.removeChild(ov);}catch(_){}};ov.querySelector('#__reqwa_ok').onclick=cleanup;ov.addEventListener('click',ev=>{if(ev.target===ov)cleanup();});}
async function deleteProduct(id){if(__frozenBlock())return;const p=products.find(x=>x.id===id);const __n=p?mpActiveMarketerCount(p):0;if(__n>0){const ar=__ar();const label=ar?(__n===1?'مسوّق نشط واحد':(__n===2?'مسوّقين نشطين':__n+' مسوّقين نشطين')):(__n+' active marketer'+(__n===1?'':'s'));alert(ar?`لا يمكن حذف هذا المنتج — لديه ${label} حالياً. انتظر حتى تكتمل طلباتهم.`:`This product can't be deleted — it has ${label} right now. Wait until those orders complete.`);return;}if(!confirm(__ar()?'هل تريد حذف هذا المنتج؟ لن يظهر للمسوّقين بعد الآن.':'Delete this product? Marketers will no longer see it.'))return;try{await window.LateenAPI.deleteProduct(id);await loadProducts();}catch(e){console.error(e);alert(__ar()?'فشل الحذف':'Failed to delete');}}
function editProduct(id){if(__frozenBlock())return;editingId=id;const p=products.find(x=>x.id===id);if(!p)return;__editLockActiveCount=mpActiveMarketerCount(p);__editLocked=__editLockActiveCount>0;openAddForm(p);}
function filterProducts(){renderProducts();}
function __bizProdCounts(){const c={all:products.length,active:0,paused:0,lowstock:0,outofstock:0};products.forEach(p=>{if(p.status==='active')c.active++;else c.paused++;const eq=__mpEffectiveQty(p);if(eq>0&&eq<=LOW_STOCK_THRESHOLD)c.lowstock++;if(eq<=0)c.outofstock++;});return c;}
function renderMpFilterChips(){const row=document.querySelector('.mp-filter-row');if(!row)return;const ar=__ar();const labels=ar?{all:'الكل',active:'نشط',paused:'متوقف',lowstock:'كمية منخفضة',outofstock:'نفدت الكمية'}:{all:'All',active:'Active',paused:'Paused',lowstock:'Low stock',outofstock:'Out of stock'};const counts=__bizProdCounts();row.querySelectorAll('.mp-filter-chip').forEach(chip=>{const f=chip.getAttribute('data-f');if(!f||!(f in labels))return;const active=chip.classList.contains('active');chip.textContent=labels[f]+(active?` (${counts[f]||0})`:'');});}
...
function dbToOrder(r){const curCode=(r.currency&&r.currency.code)||'USD';const sym=__wrapArSym((r.currency&&r.currency.symbol)||'$',curCode);const prod=products.find(p=>p.id===r.product_id);const __snap=r.product_snapshot||null;const ph=(__snap&&__snap.photos&&__snap.photos.length)?__snap.photos:(prod&&prod.photos&&prod.photos.length?prod.photos:[(prod&&prod.currency&&prod.currency.flag)||'📦']);const dt=new Date(r.created_at);const d=dt.getDate()+' '+dt.toLocaleString('en',{month:'short'})+' '+dt.getFullYear();const q=Number(r.qty)||1;const shipping=Number(r.shipping_fee)||0;const delivery=Number(r.delivery_fee)||0;const total=Number(r.unit_price)*q+shipping+delivery;const mc=r.marketer_confirmed_at?new Date(r.marketer_confirmed_at):null;const mcDate=mc?(mc.getDate()+' '+mc.toLocaleString('en',{month:'short'})+' '+mc.getFullYear()):'';return{id:'#'+(r.order_number||r.id.slice(0,8).toUpperCase()),dbId:r.id,marketerId:r.marketer_id||'',productId:r.product_id||'',source:'affiliate',paymentType:'upfront',paymentAmount:Number(r.commission)*q+Number(r.platform_fee)*q,paymentDate:d,photos:ph,productEmoji:'📦',customerName:r.customer_name||'',customerPhone:r.customer_phone||'',customerWhatsapp:r.customer_whatsapp||'',country:r.customer_country||'',city:r.customer_city||'',address:r.customer_address||'',product:(__snap&&__snap.name)?__snap.name:(prod?prod.name:'(product)'),productCode:(__snap&&__snap.code)?__snap.code:(prod?prod.code:''),sym,curCode,size:r.size||'',color:r.color||'',selectedVariants:Array.isArray(r.selected_variants)?r.selected_variants:[],qty:q,price:Number(r.unit_price)||0,shipping,delivery,total,commission:Number(r.commission)*q,platformFee:Number(r.platform_fee)*q,status:dbStatusToUi(r.status),date:d,notes:r.customer_notes||'',adminNotes:(r.refund_note||r.admin_notes||''),refundedAt:r.refunded_at||null,receiptUrl:r.receipt_url||'',marketerConfirmed:!!r.marketer_confirmed_at,marketerConfirmedDate:mcDate,_createdAt:dt,_status:r.status,_updatedAt:r.updated_at?new Date(r.updated_at):dt};}
...
function setFilter(f,el){activeFilter=f;document.querySelectorAll('.fchip').forEach(c=>c.classList.remove('active'));el.classList.add('active');applyFilters();renderOrdChips();}
function applyFilters(){const q=__normSearch(document.getElementById('search-input')?document.getElementById('search-input').value||'':'');const list=orders.filter(o=>{const mf=activeFilter==='all'||(activeFilter==='new'&&(o.status==='pending'||o.status==='approved'))||(activeFilter==='failed'&&(o.status==='failed'||o.status==='rejected'))||(activeFilter!=='new'&&activeFilter!=='failed'&&o.status===activeFilter);const s=x=>__normSearch(x);const mq=!q||s(o.id).includes(q)||s(o.customerName).includes(q)||s(__locSearchText(o.city,o.country,o.countryCode)).includes(q)||s(o.product).includes(q)||s(o.customerPhone).includes(q);return mf&&mq;});renderOrders(list);}
function __bizOrdCounts(){const c={all:orders.length,new:0,confirmed:0,delivered:0,failed:0};orders.forEach(o=>{if(o.status==='pending'||o.status==='approved')c.new++;else if(o.status==='confirmed')c.confirmed++;else if(o.status==='delivered')c.delivered++;else if(o.status==='failed'||o.status==='rejected')c.failed++;});return c;}
function renderOrdChips(){const row=document.querySelector('#pg-orders .ord-chips');if(!row)return;const ar=__ar();const labels=ar?{all:'الكل',new:'جديد',confirmed:'تم التأكيد',delivered:'تم التسليم',failed:'فشل'}:{all:'All',new:'New',confirmed:'Confirmed',delivered:'Delivered',failed:'Failed'};const counts=__bizOrdCounts();row.querySelectorAll('.fchip').forEach(chip=>{const f=chip.getAttribute('data-f');if(!f||!(f in labels))return;const active=chip.classList.contains('active');const n=counts[f]||0;const dot=(f==='new'&&n>0)?' <span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:#ef5566;margin-left:3px;vertical-align:middle;animation:blink 1.5s infinite"></span>':'';chip.innerHTML=labels[f]+(active?` (${n})`:'')+dot;});}
function updateSummary(){const nc=orders.filter(o=>o.status==='pending'||o.status==='approved').length;const banner=document.getElementById('new-banner');if(banner)banner.style.display=nc>0?'flex':'none';const bt=document.getElementById('new-banner-text');if(bt)bt.textContent=nc+' new';renderOrdChips();}
...
function computeOrdFin(o){const subtotal=o.total;const base=o.price*o.qty;const commissionPct=base>0?Math.round((o.commission/base)*100):0;const platformPct=(o.price>0&&o.qty>0)?Math.round(((o.platformFee/o.qty)/o.price)*100):0;const platformFixed=(o.price||0)<=PLATFORM_FEE_THRESHOLD;const net=base-o.commission-o.platformFee;const hasDelivery=Number(o.delivery)>0;const hasShipping=Number(o.shipping)>0;let extra=null;if(hasDelivery&&hasShipping){extra={label:'Total delivery and shipping',value:base+Number(o.delivery)+Number(o.shipping)-o.commission-o.platformFee};}else if(hasDelivery){extra={label:'Total with delivery fee',value:base+Number(o.delivery)-o.commission-o.platformFee};}else if(hasShipping){extra={label:'Total with shipping fee',value:base+Number(o.shipping)-o.commission-o.platformFee};}return{subtotal,commissionPct,platformPct,platformFixed,net,extra};}
...
function renderOrders(list){const el=document.getElementById('orders-list');if(!el)return;list=Array.isArray(list)?list:[];if(!list.length){el.innerHTML='<div class="empty-state">No orders found.</div>';return;}el.innerHTML=list.map(o=>{try{const om=n=>fmt(n,o.sym,o.curCode);const isExp=expandedId===o.id;const isImg=s=>typeof s==='string'&&/^(data:|https?:|\/)/.test(s);const photos=(o.photos&&o.photos.length)?o.photos:[o.productEmoji||'📦'];const heroSlides=photos.map(p=>isImg(p)?`<div class="hero-slide"><img src="${p}" alt="" loading="eager" decoding="async"/></div>`:`<div class="hero-slide">${p}</div>`).join('');const heroDots=photos.length>1?`<div class="hero-dots">${photos.map((_,i)=>`<div class="d${i===0?' active':''}"></div>`).join('')}</div>`:'';const stLbl=(ST[o.status]||{}).label||o.status;const __isNewBucket=(o.status==='pending'||o.status==='approved');const __newOrderLbl=__ar()?'طلب جديد':'New order';const statusTag=__isNewBucket?`<div class="status-tag tag-new"><span class="dot"></span>${__newOrderLbl}</div>`:`<div class="status-tag tag-plain ${o.status}">${stLbl}</div>`;const __prodForVariant=(typeof products!=='undefined'&&products)?products.find(p=>p.id===o.productId):null;const __vgs=(__prodForVariant&&Array.isArray(__prodForVariant.variantGroups))?__prodForVariant.variantGroups:[];const __findVariantName=(val)=>{for(const g of __vgs){for(const it of(g.items||[])){if(it&&it.val===val)return g.name||'';}}return'';};const __selVariants=(Array.isArray(o.selectedVariants)&&o.selectedVariants.length)?o.selectedVariants:[...(o.size?[{name:__findVariantName(o.size)||'Size',value:o.size}]:[]),...(o.color?[{name:__findVariantName(o.color)||'Colour',value:o.color}]:[])];const __variantRows=__selVariants.map(sv=>{let __thumb='';outer:for(const g of __vgs){for(const it of(g.items||[])){if(it&&it.val===sv.value&&it.photo){__thumb=it.photo;break outer;}}}const __thumbHtml=__thumb?`<div class="variant-thumb" onclick="event.stopPropagation();mpOpenLightbox('${String(__thumb).replace(/'/g,"\\'")}')"><img src="${__thumb}" alt="" loading="lazy"/></div>`:'';return `<div class="r variant-combo"><span class="k" data-no-i18n>${__escH(sv.name||'')}</span><div class="v variant-combo-val"><div class="variant-combo-text" data-no-i18n>${__escH(sv.value||'')} <span style="opacity:.65;font-weight:600;">×${o.qty}</span></div>${__thumbHtml}</div></div>`;}).join('');const fin=computeOrdFin(o);const codBanner=o.paymentType==='upfront'?`<div class="cod-banner"><div class="icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="3" y="7" width="18" height="12" rx="2" stroke="currentColor" stroke-width="1.8"/><path d="M3 10h18" stroke="currentColor" stroke-width="1.8"/></svg></div><div><div class="t1">Cash on delivery</div><div class="t2">Customer pays remaining balance on arrival</div></div></div>`:'';const receiptLink='';return`
      <div class="order-card ${isExp?'expanded':''} ${__isNewBucket?'new':''}" data-status="${o.status}" data-id="${o.id}">
        ...
        <div class="detail-body">
          ...
          ${o.adminNotes?`<div class="notes-box"><b>${__ar()?'ملاحظات الأدمن':'Admin notes'}:</b> <span data-no-i18n>${o.adminNotes}</span></div>`:''}
          ${receiptLink}
          ${buildOrdActions(o)}
        </div>
...
(function(){try{const c=__avRead();if(c){['user-avatar','user-avatar-p','user-avatar-o','menu-avatar'].forEach(id=>{const el=document.getElementById(id);if(el)__applyAvatarTo(el,c);});}}catch(e){}})();
async function refreshProfile(){if(!window.LateenAPI||!window.LateenAPI.getProfile)return;try{const p=await window.LateenAPI.getProfile();window.__profileData=p;const name=(p&&p.full_name)||'';const biz=(p&&p.business_name)||'';const first=name?name.split(/\s+/)[0]:(__ar()?'يا هلا':'there');const __biz=__ar()?'تاجر':'Business';const __welcome=__ar()?'أهلاً بك':'Welcome';const av=p&&p.avatar_signed_url;__avCache(av||'');['user-avatar','user-avatar-p','user-avatar-o','menu-avatar'].forEach(id=>__applyAvatarTo(document.getElementById(id),av));const g=document.getElementById('user-greet');if(g)g.textContent=(__ar()?'هلا، ':'Hey, ')+first;const s=document.getElementById('user-sub');if(s)s.textContent=biz||__biz;const mn=document.getElementById('menu-name');if(mn)mn.textContent=name||__welcome;const ms=document.getElementById('menu-sub');if(ms)ms.textContent=(biz?biz+' · ':'')+__biz;const frozen=!!(p&&p.frozen_at);const addBtn=document.getElementById('add-product-btn');if(addBtn){addBtn.disabled=frozen;addBtn.style.opacity=frozen?'0.45':'';addBtn.style.pointerEvents=frozen?'none':'';addBtn.style.cursor=frozen?'not-allowed':'';}try{document.body.classList.toggle('lateen-frozen',frozen);}catch(_){}const ar=__ar();const frozenTxt=ar?'تم تجميد الحساب مؤقتاً':'Account temporarily frozen';const banner=document.getElementById('net-frozen-banner');if(banner){banner.style.display=frozen?'block':'none';banner.textContent=frozenTxt;}const chip=document.getElementById('frozen-chip');if(chip){chip.style.display=frozen?'block':'none';chip.textContent=ar?'الحساب مجمّد':'Account frozen';}}catch(e){console.error('[Lateen] profile',e);}}
...
function __profT(){const ar=__ar();return{title:ar?'الملف الشخصي':'Profile',name:ar?'الاسم الكامل':'Full name',namePh:ar?'مثال: هيفاء وهبي':'Example: Kim Kardashian',biz:ar?'اسم المشروع':'Business name',phone:ar?'رقم الهاتف':'Phone number',email:ar?'البريد الإلكتروني':'Email',wa:ar?'واتساب او رقم هاتف إضافي':'WhatsApp or additional phone number',opt:ar?'(اختياري)':'(optional)',save:ar?'حفظ':'Save',saving:ar?'جارٍ الحفظ…':'Saving…',saved:ar?'تم الحفظ':'Saved',hint:ar?'اضغط على أيقونة الكاميرا لتغيير الصورة':'Tap the camera to change photo',tooBig:ar?'الصورة كبيرة جداً (أقصى 5 ميغابايت)':'Image too large (max 5 MB)',uploading:ar?'جارٍ رفع الصورة…':'Uploading photo…',changeBtn:ar?'تغيير':'Change',crTitle:ar?'ما الذي تريد تغييره؟':'What would you like to change?',crPhone:ar?'تغيير رقم الهاتف':'Change phone number',crEmail:ar?'تغيير البريد الإلكتروني':'Change email',crCountry:ar?'تغيير الدولة':'Change country',crSend:ar?'إرسال إلى الأدمن للتغيير':'Send to admin for change',crSent:ar?'تم فتح واتساب لإرسال طلبك إلى الأدمن':'WhatsApp opened to send your request to the admin'};}
...
function openProfile(){const t=__profT();const p=window.__profileData||{};const set=(id,v)=>{const e=document.getElementById(id);if(e)e.value=v||'';};const setTxt=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};setTxt('prof-title',t.title);setTxt('prof-lbl-name',t.name);const __pnEl=document.getElementById('prof-name');if(__pnEl)__pnEl.placeholder=t.namePh;setTxt('prof-lbl-biz',t.biz);setTxt('prof-lbl-phone',t.phone);setTxt('prof-lbl-email',t.email);setTxt('prof-change-btn',t.changeBtn);setTxt('cr-title',t.crTitle);setTxt('cr-lbl-phone',t.crPhone);setTxt('cr-lbl-email',t.crEmail);setTxt('cr-lbl-country',t.crCountry);setTxt('cr-send-txt',t.crSend);setTxt('prof-lbl-wa',t.wa);setTxt('prof-lbl-wa-opt',t.opt);setTxt('prof-save-txt',t.save);setTxt('prof-avatar-hint',t.hint);set('prof-name',p.full_name);set('prof-biz',p.business_name);const __pp=document.getElementById('prof-phone');if(__pp)__pp.textContent=__dispPhone(p.phone);const __pe=document.getElementById('prof-email');if(__pe)__pe.textContent=p.email||'';const __waSplit=__stripCC(p.whatsapp);const __waInp=document.getElementById('prof-whatsapp');if(__waInp)__waInp.value=__waSplit.num||'';const __waCcTxt=document.getElementById('prof-wacc-txt');if(__waCcTxt)__waCcTxt.textContent=__waSplit.cc||'\u200E+218\u200E';const __ctTxt=document.getElementById('prof-country-txt');if(__ctTxt)__ctTxt.textContent='🇱🇾 '+(__ar()?'ليبيا':'Libya');__applyAvatarTo(document.getElementById('prof-avatar'),p.avatar_signed_url);const ov=document.getElementById('profile-overlay');if(ov)ov.classList.add('open');const md=document.getElementById('menu-overlay');if(md)md.classList.remove('open');refreshDeleteAccountSlot();(async()=>{try{if(window.LateenAPI&&window.LateenAPI.getProfile){const fresh=await window.LateenAPI.getProfile();if(fresh){window.__profileData=fresh;set('prof-name',fresh.full_name);set('prof-biz',fresh.business_name);const __pp2=document.getElementById('prof-phone');if(__pp2)__pp2.textContent=__dispPhone(fresh.phone);const __pe2=document.getElementById('prof-email');if(__pe2)__pe2.textContent=fresh.email||'';const __waSplit2=__stripCC(fresh.whatsapp);const __waInp2=document.getElementById('prof-whatsapp');if(__waInp2)__waInp2.value=__waSplit2.num||'';const __waCcTxt2=document.getElementById('prof-wacc-txt');if(__waCcTxt2)__waCcTxt2.textContent=__waSplit2.cc||'\u200E+218\u200E';__applyAvatarTo(document.getElementById('prof-avatar'),fresh.avatar_signed_url);}}}catch(e){console.error('[Lateen] openProfile refetch failed',e);}})();}
function closeProfile(){const ov=document.getElementById('profile-overlay');if(ov)ov.classList.remove('open');}
async function pickAvatar(inp){const f=inp&&inp.files&&inp.files[0];if(!f)return;const t=__profT();if(f.size>5*1024*1024){alert(t.tooBig);inp.value='';return;}const hint=document.getElementById('prof-avatar-hint');if(hint)hint.textContent=t.uploading;try{const url=await window.LateenAPI.uploadAvatar(f);const av=document.getElementById('prof-avatar');__applyAvatarTo(av,url);if(window.__profileData)window.__profileData.avatar_signed_url=url;await refreshProfile();if(hint)hint.textContent=t.hint;}catch(e){console.error(e);alert(__ar()?'فشل رفع الصورة':'Upload failed');if(hint)hint.textContent=t.hint;}finally{inp.value='';}}
async function saveProfile(){const t=__profT();const btn=document.getElementById('prof-save');const txt=document.getElementById('prof-save-txt');const name=(document.getElementById('prof-name')||{}).value||'';const biz=(document.getElementById('prof-biz')||{}).value||'';const waNum=(document.getElementById('prof-whatsapp')||{}).value||'';const waDigits=waNum.replace(/\D/g,'');const ar=(typeof __ar==='function'&&__ar());if(waDigits&&!/^09[1-4]\d{7}$/.test(waDigits)){alert(ar?'رقم الهاتف يجب أن يكون 10 أرقام ويبدأ بـ 091 أو 092 أو 093 أو 094':'Phone must be 10 digits and start with 091, 092, 093, or 094.');return;}const __mainPhoneDigits=(typeof __stripCC==='function'?__stripCC((window.__profileData||{}).phone):{}).num||'';if(waDigits&&__mainPhoneDigits&&waDigits===__mainPhoneDigits){alert(ar?'لا يمكن أن يكون رقم الهاتف الإضافي نفس رقم الهاتف الأساسي':'Additional phone number can\'t be the same as your phone number.');return;}const wa=waDigits?('+218'+waDigits):'';const patch={full_name:name.trim()||null,business_name:biz.trim()||null,whatsapp:wa.trim()||null};if(btn){btn.disabled=true;btn.style.opacity='.7';}if(txt)txt.textContent=t.saving;try{await window.LateenAPI.updateProfile(patch);await refreshProfile();if(txt)txt.textContent=t.saved;setTimeout(()=>{if(txt)txt.textContent=t.save;if(btn){btn.disabled=false;btn.style.opacity='1';}closeProfile();},700);}catch(e){console.error(e);alert(__ar()?'فشل الحفظ':'Save failed');if(txt)txt.textContent=t.save;if(btn){btn.disabled=false;btn.style.opacity='1';}}}
...
async function refreshBizNotifications(){
  ...
  const __renderNotif=(n)=>{
    ...
    const expandable=n.kind==='new_order'||n.kind==='order_refunded'||isAdminMsg;
    const color=n.kind==='new_order'?'#34c77b':(n.kind==='order_refunded'?'#E24B4A':(n.kind==='product_review'?'#e9b949':'#7f77dd'));
    ...
    if(expandable&&n.data){
      ...
      if(d){
        ...
        const photo=(!hasPhoto&&photoUrl&&/^(https?:|data:|\/)/.test(String(photoUrl)))?`<div style="margin:-2px 0 10px 0"><img src="${esc(photoUrl)}" alt="" style="width:100%;max-height:220px;object-fit:contain;background:#0d0d0d;border-radius:10px;display:block"/></div>`:'';
        ...
        detailsHtml=`<div class="notif-details-box">
          ${photo}
          ${adminMsgContent}
          ${row(tr('Order Code','كود الطلبيه'),d.order_code)}
          ${row(tr('Product','المنتج'),d.product_name,true)}
          ${row(tr('Qty','الكمية'),d.qty)}
          ${row(tr('Customer','الزبون'),d.customer_name,true)}
          ${(function(){var p=__splitCC(d.customer_phone);return row(tr('Phone','الهاتف'),p.cc?(p.cc+' | '+p.num):p.num);})()}
          ${d.customer_whatsapp?(function(){var w=__splitCC(d.customer_whatsapp);return row(tr('WhatsApp or additional phone number','واتساب أو رقم هاتف إضافي'),w.cc?(w.cc+' | '+w.num):w.num);})():''}
          ${row(tr('City','المدينة'),d.customer_city)}
          ${row(tr('Country','الدولة'),d.customer_country)}
          ${row(tr('Address','العنوان'),d.customer_address,true)}
          ${(Array.isArray(d.selected_variants)&&d.selected_variants.length)?d.selected_variants.map(sv=>`<div style="display:flex;justify-content:space-between;gap:10px;padding:4px 0;font-size:12px"><span data-no-i18n style="color:var(--color-text-secondary)">${esc(sv.name||'')}</span><span data-no-i18n style="color:var(--color-text-primary);text-align:right">${esc(sv.value||'')}</span></div>`).join(''):(row(tr('Size','المقاس'),d.size,true)+row(tr('Colour','اللون'),d.color,true))}
          ...
          ${(n.kind==='order_refunded'&&(d.admin_comment||d.admin_note))?`<div style="margin-top:6px;padding:8px 10px;border-radius:8px;background:rgba(226,75,74,0.09);border:0.5px solid rgba(226,75,74,0.3);color:#f0a3a2;font-size:11.5px"><b>${tr('Admin notes','ملاحظات الأدمن')}:</b> <span data-no-i18n>${esc(d.admin_comment||d.admin_note)}</span></div>`:''}
        </div>`;
      }
    }
        ...
        <div class="notif-top" onclick="__notifToggle(this)">
          ...
          <div class="notif-row-text">
            <div style="flex:1;min-width:0">
              <div class="notif-title"${isAdminMsg?' data-no-i18n':''}>${esc(t)}</div>
              ${b?`<div class="notif-body">${esc(b)}</div>`:''}
...
async function confirmRequestDeletion(){
  ...
  try{
    window.__delAccStatus=await window.LateenAPI.requestAccountDeletion('business');
    renderDeleteAccountModal();
    renderDeleteAccountSlot();
