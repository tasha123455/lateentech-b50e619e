...
function hydrateVariantGroups(p){const __exPh=__ar()?'مثال: اللون أو المقاس':'Example: Color or Size';const __valPh=__ar()?'مثال: أسود أو XL':'Black or XL';const __norm=v=>({id:variantIdCounter++,val:typeof v==='string'?v:(v&&v.val)||'',qty:(v&&typeof v==='object'&&v.qty)||'',photo:(v&&typeof v==='object'&&v.photo)||''});if(!p)return[{id:groupIdCounter++,name:'',placeholder:__exPh,valPlaceholder:__valPh,items:[{id:variantIdCounter++,val:'',qty:'',photo:''}]}];if(p.variantGroups&&p.variantGroups.length)return p.variantGroups.map(g=>({id:groupIdCounter++,name:g.name,placeholder:'Variant name',valPlaceholder:__valPh,items:(g.items||[]).map(__norm)}));const out=[];if(p.sizes&&p.sizes.length)out.push({id:groupIdCounter++,name:__ar()?'المقاسات':'Sizes',placeholder:__exPh,valPlaceholder:__valPh,items:p.sizes.map(__norm)});if(p.colors&&p.colors.length)out.push({id:groupIdCounter++,name:__ar()?'الألوان':'Colours',placeholder:__exPh,valPlaceholder:__valPh,items:p.colors.map(__norm)});return out;}
...
function mpAnalyticsBody(p,sel){
  ...
  return `
    <div class="mp-stat-grid">
      ...
      <div class="mp-stat-tile"><div class="mp-stat-label">${__ar()?'المسوّقون النشطون':'Active marketers'}</div><div class="mp-stat-val">${activeMarketers}</div></div>
    </div>
