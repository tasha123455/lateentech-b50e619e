    ...
    if(expandable&&n.data){
      ...
      if(d){
        ...
        detailsHtml=`<div class="notif-details-box">
          ...
          ${(n.kind==='order_refunded'&&(d.admin_comment||d.admin_note))?`<div style="margin-top:6px;padding:8px 10px;border-radius:8px;background:rgba(226,75,74,0.09);border:0.5px solid rgba(226,75,74,0.3);color:#f0a3a2;font-size:11.5px"><b>${tr('Admin note','ملاحظة الأدمن')}:</b> <span data-no-i18n>${esc(d.admin_comment||d.admin_note)}</span></div>`:''}
        </div>`;
      }
    }
