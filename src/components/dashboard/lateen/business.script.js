...
function updateConversionNote(){const note=document.getElementById('conversion-note');if(!note)return;const price=parseFloat(document.getElementById('f-price').value)||0;const code=selectedCurrency?selectedCurrency.code:'LYD';const ar=__ar();if(commMode==='pct'){const pct=parseFloat(document.getElementById('f-comm-pct').value)||0;const amt=pctOf(price,pct);note.innerHTML=ar?`مبلغ ربح المسوق: <b>${__money(amt,'د.ل','LYD')}</b> لكل قطعه`:`Fixed equivalent at current price: <b>${amt.toFixed(2)} ${code}</b> per sale`;}else{const fixed=parseFloat(document.getElementById('f-comm-fixed').value)||0;const pct=price?((fixed/price)*100):0;note.innerHTML=ar?`نسبه ربح المسوق : <b>${pct.toFixed(1)}%</b> لكل قطعه`:`Percentage equivalent at current price: <b>${pct.toFixed(1)}%</b> per sale`;}}
function updateProductSummaryCard(){const price=parseFloat(document.getElementById('f-price').value)||0;const pct=parseFloat(document.getElementById('f-comm-pct').value)||0;const fixed=parseFloat(document.getElementById('f-comm-fixed').value)||0;const code=selectedCurrency?selectedCurrency.code:'LYD';const codeLbl=(__ar()&&code==='LYD')?'د.ل':code;const fmtSum=(n)=>(__ar()&&code==='LYD')?__money(n,'د.ل','LYD'):`${n.toFixed(2)} ${codeLbl}`;const{comm,platform,commPct}=calcFees(price,pct,fixed,commMode);const total=price-comm-platform;const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};set('sum-price',fmtSum(price));set('sum-marketer-label',__ar()?`عمولة المسوق (${commPct}%)`:`Marketer commission (${commPct}%)`);set('sum-marketer-amt',fmtSum(comm));set('sum-platform-amt',fmtSum(platform));set('sum-total',fmtSum(total));}
