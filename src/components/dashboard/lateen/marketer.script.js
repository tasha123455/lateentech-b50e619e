...
function __freeLbl(){return (typeof __ar==='function'&&__ar())?'مجاني':'Free';}
function __freeOr(n,html){return Number(n||0)===0?('<b style="color:#34c77b">'+__freeLbl()+'</b>'):html;}
...
function __normSearch(s){if(s==null)return'';let t=String(s);try{t=t.normalize('NFKC');}catch(e){}
...
function rr(){const rw=document.getElementById('rw');if(!rw)return;const si=document.getElementById('si');const q=((si&&si.value)||'').trim();const fctArr=Array.isArray(fct)?fct:(fct?[fct]:[]);const filtering=!!q||!!fc||!!fctArr.length||!!activeCatGroup||!!activeCatSub;if(filtering){rw.style.display='none';return;}const saved=P.filter(p=>p.sv);if(!saved.length){rw.style.display='none';return;}const cats=new Set();saved.forEach(p=>{if(!p.cat)return;cats.add(p.cat);const sec=CATEGORY_DATA.find(s=>s.items.includes(p.cat));if(sec)sec.items.forEach(it=>cats.add(it));});const recs=P.filter(p=>!p.sv&&__productHasStock(p)&&cats.has(p.cat)).slice(0,4);if(!recs.length){rw.style.display='none';return;}rw.style.display='block';document.getElementById('rg').innerHTML=recs.map(cd).join('');}
...
${gallery}
${descBlock}
<div class="pd-earn"><div class="pd-earn-lbl">${t.earnLbl}</div><div class="pd-earn-val">${__moneySH(earn,s,cc)}</div><div class="pd-earn-divider"></div><div class="pd-earn-rows"><div class="pd-earn-row"><span class="pd-earn-row-lbl">${t.commission}</span><span class="pd-earn-row-val pu">${__pctTxt(p.pct)}%</span></div><div class="pd-earn-row"><span class="pd-earn-row-lbl">${t.platFee}</span><span class="pd-earn-row-val">${platDisplay}</span></div><div class="pd-earn-row"><span class="pd-earn-row-lbl">${t.deposit}</span><span class="pd-earn-row-val">${__moneySH(fee,s,cc)}</span></div></div></div>
...
document.getElementById('ds').innerHTML=html;__pdCurRating=0;__revPhotoUrl='';__pdPhotos=photos;__pdZonesOpen=false;__pdZoneOpen=null;__pdStockOpen=false;renderReviews(p.id);loadReviews(p.id);const ov=document.getElementById('dov');ov.classList.add('open');try{document.body.style.overflow='hidden';document.documentElement.style.overflow='hidden';}catch(e){}const ds=document.getElementById('ds');if(ds)ds.scrollTop=0;try{if(window.LateenAPI&&window.LateenAPI.activeMarketersCounts){window.LateenAPI.activeMarketersCounts([p.id]).then(m=>{const el=document.getElementById('pdActMkt');if(el&&did===p.id)el.textContent=String(m[p.id]||0);}).catch(()=>{const el=document.getElementById('pdActMkt');if(el)el.textContent='0';});}}catch(e){}}
...
/* Reviews (product_reviews table, DB-backed) */
let __pdCurRating=0;
let __revPhotoUrl='';
const __revCache={};
...
async function submitReview(pid){const t=__pdT();const ta=document.getElementById('pd-rev-text');const txt=(ta&&ta.value||'').trim();const rating=__pdCurRating;if(!rating){alert(__ar()?'اختر تقييماً من 1 إلى 5':'Pick a rating from 1 to 5');return;}const btn=document.getElementById('pd-rev-submit');if(btn){btn.disabled=true;btn.textContent=__ar()?'جارِ الإرسال…':'Sending…';}try{if(!window.LateenAPI||!window.LateenAPI.upsertProductReview)throw new Error('no api');await window.LateenAPI.upsertProductReview(pid,rating,txt,__revPhotoUrl||'');try{let myAvatar='';try{if(window.LateenAPI&&window.LateenAPI.getProfile){const prof=await window.LateenAPI.getProfile();myAvatar=(prof&&prof.avatar_signed_url)||'';}}catch(e){}if(window.LateenAPI&&window.LateenAPI.notifyProductReview){await window.LateenAPI.notifyProductReview(pid,rating,txt,__revPhotoUrl||'',myAvatar||'');}}catch(e){console.error('[Lateen] notifyProductReview',e);}if(ta)ta.value='';setRevStar(0);removeRevPhoto();await loadReviews(pid);if(btn){btn.disabled=false;btn.textContent=t.revBtn;}}catch(e){console.error('[Lateen] submitReview',e);alert(__ar()?'تعذر إرسال التقييم':'Could not submit review');if(btn){btn.disabled=false;btn.textContent=t.revBtn;}}}
...
/* Legacy size/color are derived from the variant GROUP NAME, never from the
   group's position. Businesses can order/name their variant groups freely. */
const __SIZE_GROUP_NAMES=['size','sizes','مقاس','المقاس','مقاسات','المقاسات','حجم','الحجم'];
const __COLOR_GROUP_NAMES=['color','colour','colors','colours','لون','اللون','الوان','الألوان','الالوان'];
function __normGName(n){return String(n||'').trim().toLowerCase();}
function __isSizeGroup(n){return __SIZE_GROUP_NAMES.indexOf(__normGName(n))!==-1;}
function __isColorGroup(n){return __COLOR_GROUP_NAMES.indexOf(__normGName(n))!==-1;}
function __syncLegacyVariantFields(){selectedSize='';selectedColor='';Object.keys(selectedVariants||{}).forEach(n=>{const v=selectedVariants[n]||'';if(!v)return;if(__isSizeGroup(n))selectedSize=v;else if(__isColorGroup(n))selectedColor=v;});}
function onVariantSelect(sel){const name=sel.getAttribute('data-vgn')||'';const val=sel.value||'';selectedVariants[name]=val;__syncLegacyVariantFields();persistOpenDraft();updateSubmitState();}
...
function selectVariant(type,val,el){const g=type==='size'?'size-chips':'color-chips';document.querySelectorAll('#'+g+' .vchip').forEach(c=>c.classList.remove('selected'));el.classList.add('selected');selectedVariants[type==='size'?((typeof __ar==='function'&&__ar())?'المقاس':'Size'):((typeof __ar==='function'&&__ar())?'اللون':'Colour')]=val;__syncLegacyVariantFields();persistOpenDraft();updateSubmitState();}
...
function __profT(){const ar=__ar&&__ar();return{title:ar?'الملف الشخصي':'Profile',name:ar?'الاسم الكامل':'Full name',namePh:ar?'مثال: هيفاء وهبي':'Example: Kim Kardashian',phone:ar?'رقم الهاتف':'Phone number',email:ar?'البريد الإلكتروني':'Email',wa:ar?'واتساب او رقم هاتف إضافي':'WhatsApp or additional phone number',opt:ar?'(اختياري)':'(optional)',save:ar?'حفظ':'Save',saving:ar?'جارٍ الحفظ…':'Saving…',saved:ar?'تم الحفظ':'Saved',hint:ar?'اضغط على أيقونة الكاميرا لتغيير الصورة':'Tap the camera to change photo',tooBig:ar?'الصورة كبيرة جداً (أقصى 5 ميغابايت)':'Image too large (max 5 MB)',uploading:ar?'جارٍ رفع الصورة…':'Uploading photo…',changeBtn:ar?'تغيير':'Change',crTitle:ar?'ما الذي تريد تغييره؟':'What would you like to change?',crPhone:ar?'تغيير رقم الهاتف':'Change phone number',crEmail:ar?'تغيير البريد الإلكتروني':'Change email',crCountry:ar?'تغيير الدولة':'Change country',crSend:ar?'إرسال إلى الأدمن للتغيير':'Send to admin for change',crSent:ar?'تم فتح واتساب لإرسال طلبك إلى الأدمن':'WhatsApp opened to send your request to the admin'};}
...
function openChangeRequest(){const t=__profT();const setTxt=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};setTxt('cr-title',t.crTitle);setTxt('cr-lbl-phone',t.crPhone);setTxt('cr-lbl-email',t.crEmail);setTxt('cr-lbl-country',t.crCountry);setTxt('cr-send-txt',t.crSend);['cr-phone','cr-email','cr-country'].forEach(id=>{const el=document.getElementById(id);if(el)el.checked=false;});updateChangeRequestBtn();const ov=document.getElementById('change-request-overlay');if(ov)ov.classList.add('open');}
function closeChangeRequest(){const ov=document.getElementById('change-request-overlay');if(ov)ov.classList.remove('open');}
function updateChangeRequestBtn(){const any=['cr-phone','cr-email','cr-country'].some(id=>{const el=document.getElementById(id);return el&&el.checked;});const btn=document.getElementById('cr-send-btn');if(btn){btn.disabled=!any;btn.style.opacity=any?'1':'.5';}}
function sendChangeRequest(){const ar=(typeof __ar==='function'&&__ar());const items=[];if((document.getElementById('cr-phone')||{}).checked)items.push(ar?'رقم الهاتف':'Phone number');if((document.getElementById('cr-email')||{}).checked)items.push(ar?'البريد الإلكتروني':'Email');if((document.getElementById('cr-country')||{}).checked)items.push(ar?'الدولة':'Country');if(!items.length)return;const p=window.__profileData||{};const name=p.full_name||'';const lines=ar?['مرحباً، أرغب بطلب تغيير في بيانات حسابي على لاتين.','الاسم: '+name,'الهاتف الحالي: '+(p.phone||'-'),'البريد الحالي: '+(p.email||'-'),'أريد تغيير: '+items.join('، ')]:['Hello, I would like to request a change to my Lateen account.','Name: '+name,'Current phone: '+(p.phone||'-'),'Current email: '+(p.email||'-'),'I want to change: '+items.join(', ')];const msg=encodeURIComponent(lines.join('\n'));window.open('https://wa.me/218915756638?text='+msg,'_blank');closeChangeRequest();alert(__profT().crSent);}
  ...
  root.innerHTML=list.map(n=>{
    ...
    if(expandable){
      ...
      if(!d) d={};
      {
        ...
        const adminNote=adminNoteText?`<div style="margin-top:8px;padding:8px 10px;border-radius:8px;background:#2a1a1a;color:#f0c0c0;font-size:11px"><b>${__t('Admin notes','ملاحظات الأدمن')}:</b> <span data-no-i18n>${esc(adminNoteText)}</span></div>`:'';
...
/* No cancel handling needed: each tap creates a throw-away <input> and no
   "resume" state is persisted, so a dismissed picker leaves nothing behind. */
