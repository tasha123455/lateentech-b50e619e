...
function __freeLbl(){return (typeof __ar==='function'&&__ar())?'مجاناً':'Free';}
...
function rr(){const rw=document.getElementById('rw');if(!rw)return;const si=document.getElementById('si');const q=((si&&si.value)||'').trim();const fctArr=Array.isArray(fct)?fct:(fct?[fct]:[]);const filtering=!!q||!!fc||!!fctArr.length||!!activeCatGroup||!!activeCatSub;if(filtering){rw.style.display='none';return;}const saved=P.filter(p=>p.sv);if(!saved.length){rw.style.display='none';return;}const cats=new Set(saved.map(p=>p.cat));const recs=P.filter(p=>!p.sv&&__productHasStock(p)&&cats.has(p.cat)).slice(0,4);if(!recs.length){rw.style.display='none';return;}rw.style.display='block';document.getElementById('rg').innerHTML=recs.map(cd).join('');}
