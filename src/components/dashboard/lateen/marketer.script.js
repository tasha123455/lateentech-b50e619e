...
function go(){renderCatFilters();const si=document.getElementById('si');const q=__normSearch(si?si.value||'':'');const fctArr=Array.isArray(fct)?fct:(fct?[fct]:[]);let l=P.filter(p=>{if(!__productHasStock(p))return false;const mq=!q||__normSearch(p.n).includes(q)||__normSearch(p.code||'').includes(q)||__catSearchText(p.cat).includes(q);const mc=!fc||!!p.d[fc];const mct=!fctArr.length||(fc&&p.d[fc]&&fctArr.some(ct=>!!p.d[fc].c[ct]));const mcat=!activeCatGroup||(activeCatSub?p.cat===activeCatSub:(CATEGORY_DATA.find(s=>s.group===activeCatGroup)||{items:[]}).items.includes(p.cat));return mq&&mc&&mct&&mcat;});if(fs==='ch')l.sort((a,b)=>b.pct-a.pct);else if(fs==='cl')l.sort((a,b)=>a.pct-b.pct);else if(fs==='ph')l.sort((a,b)=>b.pr-a.pr);else if(fs==='pl')l.sort((a,b)=>a.pr-b.pr);rg2(l);if(fs==='ch'||fs==='cl'||fs==='ph'||fs==='pl'){const rw=document.getElementById('rw');if(rw)rw.style.display='none';}else{rr();}ub();}
