...
function __mpEffectiveQty(p){
  ...
  // Each variant group (Size, Colour, ...) slices the SAME physical stock,
  // not a separate pool — so the true total is the smallest group total
  // (the bottleneck), never the sum of all groups. If any one group is
  // fully depleted (e.g. all sizes at 0), the bottleneck is 0 and the whole
  // product reads as out of stock, even if another group still has stock.
