...
function NotificationButton() {
  return (
    <button className="relative flex h-9 w-9 items-center justify-center rounded-full border border-border bg-surface" aria-label={useT()("Notifications")}>
...
export function SignOutInline() {
  ...
  return <button onClick={signOut} className="text-xs text-text-2 underline">{useT()("Sign out")}</button>;
}
