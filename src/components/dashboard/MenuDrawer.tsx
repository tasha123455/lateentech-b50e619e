...
export function MenuDrawer({ open, onClose, name, subtitle }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-30 flex">
      <button onClick={onClose} className="absolute inset-0 bg-black/60" aria-label={t("Close menu")} />
      <aside className="absolute end-0 top-0 bottom-0 flex w-[78%] flex-col rounded-s-2xl border-s border-border-strong bg-surface p-6">
        <button onClick={onClose} className="self-end text-text-2" aria-label={t("Close menu")}>✕</button>
        ...
        <MenuItem label={t("Account settings")} tone="green" />
        <MenuItem label={t("Payouts")} tone="purple" />
        <MenuItem label={t("Help & support")} tone="green" />
...
function MenuItem({ label, tone }: { label: string; tone: "green" | "purple" }) {
  ...
  return (
    <button className="flex w-full items-center gap-3 border-b border-border py-3.5 text-start">
