...
/* Changing somebody's sign-in email.
 *
 * This has to run on the server. The address lives on auth.users, and the only
 * safe way to move it is Supabase's admin API, which needs the service role
 * key — a key that must never reach a browser. So the browser sends what it
 * wants done and this route decides whether it is allowed.
 *
...
export const Route = createFileRoute("/api/admin/account-email")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!expected) {
          return json({ error: "unavailable", message: "Email changes are not enabled on this environment." }, 503);
        }
        ...
        /* No email means this was only the unlock step: the code is good, so
           the box may open. Nothing has changed yet. */
        if (body.email === undefined) return json({ ok: true, unlocked: true });
        ...
        if (userId === callerId) {
          return json({ error: "forbidden", message: "Use your own account settings for your own email." }, 403);
        }
        ...
        if (target.user.email && target.user.email.toLowerCase() === email) {
          return json({ ok: true, email, unchanged: true });
        }

        /* email_confirm marks it verified straight away. Without it the account
           sits in limbo waiting for a confirmation link sent to an address the
           person may well have lost — which is the exact situation this whole
        ...
        console.log("[admin] sign-in email changed", { by: callerId, user: userId });
