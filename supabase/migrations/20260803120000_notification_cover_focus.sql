-- Notifications frame the cover photo the way its owner framed it.
--
-- A business owner drags their cover photo when listing a product, choosing
-- which part of it survives being cropped to a square. That choice was being
-- honoured on the browse grid and nowhere else, so the same product showed the
-- owner's framing there and the dead centre of the photo on every order card,
-- on the public link, and in the notification.
--
-- The card can only frame what it was told. A notification carries its own
-- copy of the order rather than reading the product back at display time, so
-- the two numbers have to travel with the photo they belong to.
--
-- Generated from each function's current definition with the pair added next
-- to 'product_photo'; nothing else about any of them changes.
